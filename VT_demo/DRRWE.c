...
PPROCESS_List Dr_AddProcessToList(PEPROCESS Process) {
  ...
  TempItem = (PPROCESS_List)ExAllocatePoolWithTag(NonPagedPool,
                                                  sizeof(PROCESS_List), 'drrp');
  if (!TempItem) {
    return FALSE;
  }
...
PTHREAD_dr_List Dr_AddThreadStructToList(PPROCESS_List DrRwListItem,
                                         PTHREAD_dr_List Struct) {

  PTHREAD_dr_List TempItem;
  TempItem = (PTHREAD_dr_List)ExAllocatePoolWithTag(
      NonPagedPool, sizeof(THREAD_dr_List), 'drrt');
  if (!TempItem) {
    return FALSE;
  }
  ...
  ExInterlockedInsertTailList(&DrRwListItem->ThreadList, &TempItem->TList,
                              &DrRwListItem->loacl_lock);
  if (TempItem != NULL) {
...
PTHREAD_dr_List Dr_FindThreadContextByThreadList(PPROCESS_List DrRwListItem,
                                                 PETHREAD Thread) {
  KIRQL OldIrql;
  PLIST_ENTRY Entry;
...
PTHREAD_dr_List Dr_UpdataThreadContextByThreadList(PPROCESS_List DrRwListItem,
                                                   PETHREAD Thread,
                                                   PTHREAD_dr_List UpData) {
  KIRQL OldIrql;
  PLIST_ENTRY Entry;
  THREAD_dr_List *TempItem = NULL;
  THREAD_dr_List *DFind = NULL;
...
VOID NTAPI Dr_ExFreeTheadListItem(PPROCESS_List DrRwListItem,
                                  PTHREAD_dr_List Struct) {
  KIRQL OldIrql;
  KeAcquireSpinLock(&DrRwListItem->loacl_lock, &OldIrql);
