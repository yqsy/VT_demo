...
typedef NTSTATUS(__fastcall *pfNtUserFindWindowEx)(HWND a1, HWND a2,
                                                   PUNICODE_STRING a3,
                                                   PUNICODE_STRING a4,
                                                   ULONG a5);
typedef HANDLE(__fastcall *pfNTUSERGETFOREGROUNDWINDOW)(VOID);
typedef NTSTATUS(__fastcall *pfNtUserBuildHwndList)(
    IN HDESK hdesk, IN HWND hwndNext, IN ULONG fEnumChildren, IN DWORD idThread,
...
typedef __int64(__fastcall *pfNtUserChildWindowFromPointEx)(
    _In_ HWND hwndParent, _In_ POINT pt, _In_ UINT uFlags);
typedef __int64(__fastcall *pfNtUserRealChildWindowFromPoint)(
    _In_ HWND hwndParent, _In_ POINT ptParentClientCoords);
typedef __int64(__fastcall *pfNtUserWindowFromPhysicalPoint)(__int64 a1);
typedef __int64(__fastcall *pfNtUserMessageCall)(__int64 a1, unsigned int a2,
                                                 __int64 a3, __int64 a4,
                                                 __int64 a5, int a6);
typedef __int64(__fastcall *pfNtUserGetClassName)(HWND hWnd, LPTSTR IpClassName,
                                                  int nMaxCount);
...
typedef HANDLE(__fastcall *NTUSERQUERYWINDOW)(IN HWND WindowHandle,
                                              IN ULONG TypeInformation);

typedef ULONG64(__fastcall *NTUSERPOSTMESSAGE)(ULONG64 hWnd, INT Msg,
                                               ULONG32 wParam, ULONG32 lParam);
NTSYSAPI NTSTATUS NTAPI ZwQuerySystemInformation(
    IN ULONG SystemInformationClass, OUT PVOID SystemInformation,
  ...
  for (i = 0; i < ModuleCount; i++) {
    if ((ULONG64)(pSystemModuleInformation->Module[i].Base) >
      ...
      pDrvName = pSystemModuleInformation->Module[i].ImageName +
                 pSystemModuleInformation->Module[i].ModuleNameOffset;
...
VOID ModifySSSDT(ULONG64 Index, ULONG64 Address, CHAR ParamCount) {
  ...
  pWin32k =
      (PKSERVICE_TABLE_DESCRIPTOR)((ULONG64)KeServiceDescriptorTableShadow +
                                   sizeof(KSERVICE_TABLE_DESCRIPTOR)); // 4*8
  W32pServiceTable = (ULONGLONG)(pWin32k->ServiceTableBase);
  qwTemp = W32pServiceTable + 4 * (Index - 0x1000);
  dwTemp = (LONG)(Address - W32pServiceTable);
  dwTemp =
      dwTemp
      << 4; // DbgPrint("*(PLONG)qwTemp: %x, dwTemp: %x",*(PLONG)qwTemp,dwTemp);
...
VOID HOOK_SSSDT(ULONG64 FunctionId, ULONG64 ProxyFunctionAddress,
                CHAR ParamCount) // return OriFunctionAddress
{
...
VOID UNHOOK_SSSDT(ULONG64 FunctionId, ULONG64 OriFunctionAddress,
                  CHAR ParamCount) {
  ModifySSSDT(FunctionId, (ULONG64)OriFunctionAddress, ParamCount);
  DbgPrint("UNHOOK_SSSDT OK!");
}

NTSTATUS __fastcall MyNtUserFindWindowEx(HWND a1, HWND a2, PUNICODE_STRING a3,
                                         PUNICODE_STRING a4, ULONG a5) {
...
HANDLE __fastcall MyNtUserQueryWindow(IN HWND WindowHandle,
                                      IN ULONG TypeInformation) {
  p_save_handlentry Padd = NULL;
...
NTSTATUS __fastcall MyNtUserBuildHwndList(IN HDESK hdesk, IN HWND hwndNext,
                                          IN ULONG fEnumChildren,
                                          IN DWORD idThread, IN UINT cHwndMax,
                                          OUT HWND *phwndFirst,
  ...
  if (Padd == NULL) {
    ...
    result = orgNtUserBuildHwndList(hdesk, hwndNext, fEnumChildren, idThread,
                                    cHwndMax, phwndFirst, pcHwndNeeded);
  ...
  return orgNtUserBuildHwndList(hdesk, hwndNext, fEnumChildren, idThread,
                                cHwndMax, phwndFirst, pcHwndNeeded);
}
            ...
            if (IS_HOOK == 1 && !FindDebugInfoByEprocess(IoGetCurrentProcess()))
            {
                ...
                while (i < *pcHwndNeeded)
                {
                    ...
                    if (GetDebugInfoByPid(ProcessId))
                    {
                        ...
                        RtlCopyMemory(&phwndFirst[i + 1], &phwndFirst[i],
   *pcHwndNeeded - i);
                        // 最后一项清0
                        phwndFirst[*pcHwndNeeded - 1] = 0;
                        // 总数减1
...
__int64 __fastcall MyNtUserMessageCall(__int64 a1, unsigned int a2, __int64 a3,
