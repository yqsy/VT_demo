...
VOID HvmpHVCallbackDPC(PRKDPC Dpc, PVOID Context, PVOID SystemArgument1,
  ...
  if (ARGUMENT_PRESENT(Context)) {
    ...
    g_Data->CPUVendor == CPU_Intel ? IntelSubvertCPU(pVCPU, Context)
                                   : AMDSubvertCPU(pVCPU, Context);
  } else {
    // Tear down the virtual processor
    g_Data->CPUVendor == CPU_Intel ? IntelRestoreCPU(pVCPU)
                                   : AMDRestoreCPU(pVCPU);
  }
