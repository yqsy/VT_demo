...
NTKERNELAPI
BOOLEAN
PsIsSystemThread(_In_ PETHREAD Thread);
...
BOOLEAN __fastcall IstThreadStub(PETHREAD OldThread, PETHREAD NewThread) {
  ...
  if ((PsIsSystemThread(OldThread) == TRUE) &&
      (PsIsSystemThread(NewThread) == TRUE)) {
    return TRUE;
  }
...
VOID EPTHOOK_SwapContext() { PHHook(SwapContext, MySwapContext); }

VOID EPTUNHOOK_SwapContext() { PHRestore(SwapContext); }
VOID HOOKSwapContext() {
  pslp_head_n_byte30 = HookKernelApi(SwapContext, (PVOID)&MySwapContext,
                                     &ori_pslp30, &pslp_patch_size30);
}
