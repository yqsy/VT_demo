...
#ifndef _DBGFUNC
#define _DBGFUNC
NTSTATUS PsLookupProcessByProcessId(_In_ HANDLE ProcessId,
                                    _Out_ PEPROCESS *Process);
NTKERNELAPI NTSTATUS PsLookupThreadByThreadId(_In_ HANDLE ThreadId,
                                              _Outptr_ PETHREAD *Thread);
VOID SetDbgMsgNotify(BOOLEAN IsLoad);
extern p_save_handlentry PmainList;
typedef NTSTATUS(__fastcall *pfDbgkpSetProcessDebugObject)(
    IN PEPROCESS_S Process, IN PDEBUG_OBJECT DebugObject, IN NTSTATUS MsgStatus,
    IN PETHREAD LastThread);

typedef NTSTATUS(__fastcall *pfMmGetFileNameForAddress)(
    PIMAGE_NT_HEADERS pnt, PUNICODE_STRING modname);
typedef NTSTATUS(__fastcall *pfDbgkpPostFakeProcessCreateMessages)(
    IN PEPROCESS_S Process, IN PDEBUG_OBJECT DebugObject,
    IN PETHREAD *pLastThread);
NTSTATUS NTAPI DbgkClearProcessDebugObject(
    IN PEPROCESS_S Process, IN PDEBUG_OBJECT SourceDebugObject OPTIONAL);
NTSTATUS __fastcall DbgkpSendApiMessage_2(IN OUT PDBGKM_MSG ApiMsg,
                                          IN BOOLEAN SuspendProcess);
BOOLEAN DbgkpSuppressDbgMsg(IN PTEB64 Teb);
VOID NTAPI DbgkpWakeTarget(IN PDEBUG_EVENT DebugEvent);
NTKERNELAPI
VOID KeStackAttachProcess(__inout PEPROCESS PROCESS,
...
typedef NTSTATUS (*OBINSERTOBJECT)(__in PVOID Object,
...
typedef NTSTATUS(__stdcall *OBCREATEOBJECT)(
    __in KPROCESSOR_MODE ProbeMode, __in POBJECT_TYPE ObjectType,
    __in POBJECT_ATTRIBUTES ObjectAttributes,
    __in KPROCESSOR_MODE OwnershipMode, __inout_opt PVOID ParseContext,
    __in ULONG ObjectBodySize, __in ULONG PagedPoolCharge,
    __in ULONG NonPagedPoolCharge, __out PVOID *Object);
typedef NTSTATUS (*OBOPENOBJECTBYPOINTER)(
    __in PVOID Object, __in ULONG HandleAttributes,
    __in_opt PACCESS_STATE PassedAccessState, __in ACCESS_MASK DesiredAccess,
    __in_opt POBJECT_TYPE ObjectType, __in KPROCESSOR_MODE AccessMode,
    __out PHANDLE Handle);

typedef NTSTATUS (*ObDuplicateObject1)(
    IN PEPROCESS_S SourceProcess, IN HANDLE SourceHandle,
    IN PEPROCESS_S TargetProcess OPTIONAL, OUT PHANDLE TargetHandle OPTIONAL,
    IN ACCESS_MASK DesiredAccess, IN ULONG HandleAttributes, IN ULONG Options,
    IN KPROCESSOR_MODE PreviousMode);
typedef PETHREAD(__fastcall *PsGetNextProcessThreadx)(PEPROCESS_S process,
                                                      PKTHREAD THREAD);
typedef NTSTATUS(__fastcall *DbgkpPostModuleMessagesx)(PEPROCESS_S process,
                                                       PKTHREAD THREAD,
                                                       PDEBUG_OBJECT debug);
typedef NTSTATUS(__fastcall *KeThawAllThreadsx)();
typedef NTSTATUS(__fastcall *PsResumeThreadx)(
    IN PETHREAD Thread, OUT PULONG PreviousSuspendCount OPTIONAL);
typedef NTSTATUS(__fastcall *PsSuspendThreadx)(
    IN PETHREAD Thread, OUT PULONG PreviousSuspendCount OPTIONAL);
typedef NTSTATUS(__fastcall *MmGetFileNameForSectionx)(
    IN PVOID Thread, OUT POBJECT_NAME_INFORMATION FileName OPTIONAL);
typedef NTSTATUS(__fastcall *PsTerminateProcessx)(IN PEPROCESS_S Process,
                                                  NTSTATUS STATUS);
...
typedef NTSTATUS(__fastcall *proxyDbgkpQueueMessage)(
    IN PEPROCESS_S Process, IN PETHREAD Thread, IN OUT PDBGKM_MSG ApiMsg,
    IN ULONG Flags, IN PDEBUG_OBJECT TargetDebugObject);
typedef NTSTATUS(__fastcall *proxyDbgkpSendApiMessage)(
...
typedef NTSTATUS(__fastcall *DbgkpPostFakeThreadMessagesx)(
    IN PEPROCESS_S Process, IN ULONG64 DebugObject, IN PETHREAD StartThread,
    OUT PETHREAD *FirstThread, OUT PETHREAD *LastThread);
...
DbgkpSectionToFileHandle(IN PVOID Section) {
  ...
  InitializeObjectAttributes(&ObjectAttributes, &FileName->Name,
...
VOID NTAPI proxyDbgkExitThread(IN NTSTATUS ExitStatus) {
  ...
  if (PsGetCurrentThread()->CrossThreadFlags &
  ...
  ApiMessage.h.u1.Length =
      sizeof(DBGKM_MSG) << 16 | (8 + sizeof(DBGKM_EXIT_THREAD));
...
VOID NTAPI proxyDbgkExitProcess(IN NTSTATUS ExitStatus) {
  DBGKM_MSG ApiMessage;
  ...
  ApiMessage.h.u1.Length =
      sizeof(DBGKM_MSG) << 16 | (8 + sizeof(DBGKM_EXIT_PROCESS));
  ApiMessage.h.u2.ZeroInit = 0;
  ApiMessage.h.u2.s2.Type = LPC_DEBUG_EVENT;
...
VOID __fastcall proxyDbgkUnMapViewOfSection(IN PEPROCESS_S PROCESS,
  ...
  if (Thread->SystemThread != TRUE && Thread->Tcb.ApcStateIndex != 0x1) {
  ...
  ApiMessage.h.u1.Length =
      sizeof(DBGKM_MSG) << 16 | (8 + sizeof(DBGKM_UNLOAD_DLL));
  ApiMessage.h.u2.ZeroInit = 0;
  ApiMessage.h.u2.s2.Type = LPC_DEBUG_EVENT;
...
VOID __fastcall proxyDbgkMapViewOfSection(IN PVOID Processs, IN PVOID Section,
  ...
  if (NtHeader) {
    ...
    LoadDll->DebugInfoFileOffset = NtHeader->FileHeader.PointerToSymbolTable;
    LoadDll->DebugInfoSize = NtHeader->FileHeader.NumberOfSymbols;
  }

  ApiMessage.h.u1.Length =
      sizeof(DBGKM_MSG) << 16 | (8 + sizeof(DBGKM_LOAD_DLL));
  ApiMessage.h.u2.ZeroInit = 0;
  ApiMessage.h.u2.s2.Type = LPC_DEBUG_EVENT;
...
BOOLEAN DbgkpSuppressDbgMsg(IN PTEB64 Teb) {
  ...
  except(EXCEPTION_EXECUTE_HANDLER) { bSuppress = FALSE; }
  return bSuppress;
}
NTSTATUS __fastcall proxyNtCreateDebugObject(
  ...
  /*
      Padd = querylist(PmainList, PsGetCurrentProcessId(),
     PsGetCurrentProcess()); if (Padd == NULL)
      {
          DbgPrint("proxyNtCreateDebugObject");
          return ori_pslp40(DebugObjectHandle, DesiredAccess, ObjectAttributes,
     Flags);
  ...
  //创建调试对象
  status =
      ObCreateObject(PreviousMode, NewDbgObject, ObjectAttributes, PreviousMode,
                     NULL, sizeof(DEBUG_OBJECT), 0, 0, (PVOID *)&DebugObject);
  ...
  except(ExSystemExceptionFilter()) { status = GetExceptionCode(); }
...
VOID SendForWarExcept_Thread() {
  ...
  ApiMessage.h.u1.Length =
      sizeof(DBGKM_MSG) << 16 | (8 + sizeof(DBGKM_CREATE_THREAD));
  ApiMessage.h.u2.ZeroInit = 0;
  ApiMessage.h.u2.s2.Type = LPC_DEBUG_EVENT;
...
BOOLEAN __fastcall MarkDbgProcess() {
  ...
  if (DbgList != NULL && MmIsAddressValid(DbgList) == TRUE &&
      DbgList->markdbg == FALSE) {
...
NTSTATUS
NTAPI
NtRemoveProcessDebug(IN HANDLE ProcessHandle, IN HANDLE DebugHandle) {
  DbgProcess dbgmsg = {0};
  PEPROCESS_S Process;
  PDEBUG_OBJECT DebugObject;
  ...
  Status = ObReferenceObjectByHandle(
      DebugHandle, DEBUG_OBJECT_ADD_REMOVE_PROCESS, NewDbgObject, PreviousMode,
...
NTSTATUS
NTAPI
proxyNtDebugContinue(IN HANDLE DebugHandle, IN PCLIENT_ID AppClientId,
...
*/
  ...
  if ((ContinueStatus != DBG_CONTINUE) &&
    ...
    Status = ObReferenceObjectByHandle(
        DebugHandle, DEBUG_OBJECT_WAIT_STATE_CHANGE, NewDbgObject, PreviousMode,
        (PVOID *)&DebugObject, NULL);
    /*  Status = ObReferenceObjectByHandle(DebugHandle,
...
NTSTATUS
__fastcall proxyDbgkOpenProcessDebugPort(IN PEPROCESS_S Process,
                                         IN KPROCESSOR_MODE PreviousMode,
  ...
  */
  ...
  Status = ObOpenObjectByPointer(DebugObject, 0, NULL, MAXIMUM_ALLOWED,
                                 NewDbgObject, PreviousMode, DebugHandle);
  if (!NT_SUCCESS(Status))
    ObDereferenceObject(DebugObject);
...
VOID proxyDbgkCopyProcessDebugPort(IN PEPROCESS_S TargetProcess,
...
VOID __fastcall DbgkpCloseObject(IN PEPROCESS_S Process, IN PVOID Object,
...
--*/
{
  ...
  //
  // Wake anyone waiting. They need to leave this object alone now as its
  // deleting
  //
  KeSetEvent(&DebugObject->EventsPresent, 0, FALSE);

  //
  // Loop over all processes and remove the debug port from any that still have
  // it. Debug port propagation was disabled by setting the delete pending flag
  // above so we only have to do this once. No more refs can appear now.
  //
  ExAcquireFastMutex(&DbgkFastMutex);
  ...
  if (Deref) {
    ...
    //
    // If the caller wanted process deletion on debugger dying (old interface)
    // then kill off the process.
    //
  ...
  /*
      for (Process = PsGetNextProcess(NULL);
          ...
          
          {
              ...
              //  DbgkpMarkProcessPeb(Process);
                  //
                  // If the caller wanted process deletion on debugger dying
     (old interface) then kill off the process.
                  //
                  if (DebugObject->Flags&DEBUG_OBJECT_KILL_ON_CLOSE) {
                      PsTerminateProcess(Process, STATUS_DEBUGGER_INACTIVE);
                  }
...
POBJECT_TYPE CreateNewObjectType(POBJECT_TYPE_S *OrigDebugObjectType) {
  ...
  ObCreateObjectType =
      (OBCREATEOBJECTTYPE)MmGetSystemRoutineAddress(&usFuncName);
  ...
  memcpy(&Object_Type_Init, &(*OrigDebugObjectType)->TypeInfo,
         sizeof(OBJECT_TYPE_INITIALIZER_S));
...
NTSTATUS __fastcall DbgkpQueueMessage_2(IN PEPROCESS_S Process,
                                        IN PETHREAD Thread,
...

{
  ...
  if (Flags & DEBUG_EVENT_NOWAIT) {
    DebugEvent = ExAllocatePoolWithQuotaTag(
        NonPagedPool | POOL_QUOTA_FAIL_INSTEAD_OF_RAISE, sizeof(*DebugEvent),
        'EgbD');
    if (DebugEvent == NULL) {
  ...
  if ((Flags & DEBUG_EVENT_NOWAIT) == 0) {
    ...
    if (NT_SUCCESS(Status)) {
      KeWaitForSingleObject(&DebugEvent->ContinueEvent, Executive, KernelMode,
                            FALSE, NULL);
...
NTSTATUS
__fastcall DbgkpSendApiMessage_2(IN OUT PDBGKM_MSG ApiMsg,
                                 IN BOOLEAN SuspendProcess) {
  NTSTATUS Status;
  ...
  // PspSetFlag(&((PEPROCESS_S)PsGetCurrentProcess())->Flags,
  // PS_PROCESS_FLAGS_CREATE_REPORTED);

  /* Send the LPC command */
  Status = DbgkpQueueMessage_2(PsGetCurrentProcess(), PsGetCurrentThread(),
                               ApiMsg, ((SuspendProcess & 0x2) << 0x5), NULL);
...
BOOLEAN
__fastcall proxyDbgkForwardException(IN PEXCEPTION_RECORD ExceptionRecord,
  ...
  */
  ...
  ApiMessage.h.u1.Length =
      sizeof(DBGKM_MSG) << 16 | (8 + sizeof(DBGKM_EXCEPTION));
  ApiMessage.h.u2.ZeroInit = 0;
  ApiMessage.h.u2.s2.Type = LPC_DEBUG_EVENT;
...
NTSTATUS DbgkpPostFakeProcessCreateMessages_2(IN PEPROCESS_S Process,
                                              IN PDEBUG_OBJECT DebugObject,
                                              IN PETHREAD *pLastThread) {
  NTSTATUS status;
  KAPC_STATE ApcState;
  ...
  //收集所有线程创建的消息
  StartThread = 0;
  status = DbgkpPostFakeThreadMessages(Process, DebugObject, StartThread,
                                       &Thread, &LastThread);
...
VOID NTAPI DbgkpConvertKernelToUserStateChange(
    IN PDBGUI_WAIT_STATE_CHANGE WaitStateChange, IN PDEBUG_EVENT DebugEvent) {
  ...
  case DbgKmExceptionApi:
    ...
    } else if ((NTSTATUS)DebugEvent->ApiMsg.Exception.ExceptionRecord
...
VOID NTAPI DbgkpOpenHandles(IN PDBGUI_WAIT_STATE_CHANGE WaitStateChange,
  ...
  if (Handle) {
    ...
    Status =
        ObDuplicateObject(PsGetCurrentProcess(), Handle, PsGetCurrentProcess(),
                          DupHandle, 0, 0, DUPLICATE_SAME_ACCESS, KernelMode);
    if (!NT_SUCCESS(Status))
      *DupHandle = NULL;
...
DbgkClearProcessDebugObject(IN PEPROCESS_S Process,
  ...
  DebugObject = Port_GetPort(Process);
  if ((DebugObject) &&
      ((DebugObject == SourceDebugObject) || (SourceDebugObject == NULL))) {
...
PVOID PsQuerySystemDllInfo(ULONG index) {
  PVOID64 DllInfo;

  DllInfo = (PVOID64)PspSystemDlls[index];
  if (DllInfo != NULL && *(PVOID *)((char *)DllInfo + 0x28) != 0) {
...
VOID DbgkSendSystemDllMessages(PETHREAD Thread, PDEBUG_OBJECT DebugObject,
                               PDBGKM_MSG ApiMsg) {
  NTSTATUS status;
  ...
  do {
    ...
    if (DllInfo != NULL) {
      ...
      if (Thread == 0) {
        ...
        if (Teb) {
          RtlStringCbCopyW(Teb->StaticUnicodeBuffer, 261 * sizeof(wchar_t),
                           DllInfo->Buffer);
      ...
      InitializeObjectAttributes(&ObjectAttr, &DllInfo->FileName,
                                 OBJ_CASE_INSENSITIVE | OBJ_FORCE_ACCESS_CHECK |
                                     OBJ_KERNEL_HANDLE,
                                 NULL, NULL);

      status = ZwOpenFile(
          &FileHandle, GENERIC_READ | SYNCHRONIZE, &ObjectAttr, &IoStackBlock,
          FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE,
          FILE_SYNCHRONOUS_IO_NONALERT);
      if (!NT_SUCCESS(status)) {
        FileHandle = NULL;
      ...
      if (Thread == NULL) {
        ...
        status = DbgkpQueueMessage_2(Process, Thread, ApiMsg,
                                     DEBUG_EVENT_NOWAIT, DebugObject);
        if (!NT_SUCCESS(status)) {
          if (FileHandle != NULL) {
            ObCloseHandle(FileHandle, KernelMode);
          }
...
NTSTATUS DbgkpPostFakeThreadMessages_2(
    ...
    PEPROCESS_S Process, PDEBUG_OBJECT DebugObject, PETHREAD StartThread,
    PETHREAD *pFirstThread, PETHREAD *pLastThread) {
  NTSTATUS status;
  ...
  for (Thread = StartThread; Thread != NULL;
    ...
    if (First && (Flags & DEBUG_EVENT_PROTECT_FAILED) == 0) {
      ...
      if (Process->SectionObject) {
        ...
        ApiMsg.CreateProcess.FileHandle =
            DbgkpSectionToFileHandle(Process->SectionObject);
      } else {
      ...
      __try {
        ...
        if (NtHeaders) {
          ...
          ApiMsg.CreateProcess.DebugInfoFileOffset =
              NtHeaders->FileHeader.PointerToSymbolTable;
          ApiMsg.CreateProcess.DebugInfoSize =
    ...
    status = DbgkpQueueMessage_2(Process, Thread, &ApiMsg, Flags, DebugObject);
...
NTSTATUS __fastcall DbgkpSetProcessDebugObject_2( //反汇编OK
    IN PEPROCESS_S Process, IN PDEBUG_OBJECT DebugObject, IN NTSTATUS MsgStatus,
    IN PETHREAD LastThread) {
  NTSTATUS Status;
  PETHREAD ThisThread;
  ...
  if (NT_SUCCESS(Status)) {
    while (1) {
      ...
      if (Thread != NULL) {
        ...
        Status = DbgkpPostFakeThreadMessages(Process, DebugObject, Thread,
  ...
  if (NT_SUCCESS(Status)) {
    if ((DebugObject->Flags & DEBUG_OBJECT_DELETE_PENDING) == 0) {
      PspSetProcessFlag(&Process->Flags, PS_PROCESS_FLAGS_NO_DEBUG_INHERIT |
                                             PS_PROCESS_FLAGS_CREATE_REPORTED);
      ObReferenceObject(
          DebugObject); // Process->NoDebugInherit 为1就表示有调试了。
    } else {
  ...
  for (Entry = DebugObject->EventList.Flink;
    ...
    if ((DebugEvent->Flags & DEBUG_EVENT_INACTIVE) != 0 &&
        DebugEvent->BackoutThread == ThisThread) {
...
NTSTATUS
__fastcall proxyNtWaitForDebugEvent(IN HANDLE DebugHandle, IN BOOLEAN Alertable,
  ...
  while (TRUE) {
    Status = KeWaitForSingleObject(&DebugObject->EventsPresent, Executive,
...
VOID GetCloseDbgtoolMsg(IN HANDLE hParentId, IN HANDLE hProcessId,
                        IN BOOLEAN bCreate) {
  p_save_handlentry Padd = NULL;
  if (!bCreate) {
...
VOID MsgCreateUnThreadMsg(_In_ HANDLE ProcessId, _In_ HANDLE ThreadId,
                          _In_ BOOLEAN Create) {
...
NTSTATUS
__fastcall proxyNtDebugActiveProcess(IN HANDLE ProcessHandle,
  ...
  Status =
      DbgkpPostFakeProcessCreateMessages_2(Process, DebugObject, &LastThread);
  Status =
      DbgkpSetProcessDebugObject_2(Process, DebugObject, Status, LastThread);
