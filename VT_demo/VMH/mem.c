...
VOID UnAllocateMemory(PVOID pMem) { ExFreePoolWithTag(pMem, VIRTDBG_POOLTAG); }
...
PVOID NTAPI MmAllocatePages(ULONG uNumberOfPages,
  ...
  PageVA =
      ExAllocatePoolWithTag(NonPagedPool, uNumberOfPages * PAGE_SIZE, ITL_TAG);
  if (!PageVA)
    return NULL;
...
PVOID NTAPI
MmAllocateContiguousPagesSpecifyCache( //分配的范围内连续的，非分页物理存储器，并将其映射到系统地址空间。
    ULONG uNumberOfPages, PPHYSICAL_ADDRESS pFirstPagePA, ULONG CacheType) {
  PVOID PageVA;
  ...
  PageVA = MmAllocateContiguousMemorySpecifyCache(uNumberOfPages * PAGE_SIZE,
                                                  l1, l2, l3, CacheType);
  if (!PageVA)
    return NULL;
...
PVOID NTAPI
MmAllocateContiguousPages( //分配的范围内连续的，非分页物理存储器，并将其映射到系统地址空间。
    ULONG uNumberOfPages, PPHYSICAL_ADDRESS pFirstPagePA) {
  //  NumberOfBytes [中]
  ...
  //最有效的物理地址调用者可以使用。
  //例如，如果一个设备能解决在第一个16兆字节的处理器的物理存储器地址范围仅位置，驱动此设备应该设置HighestAcceptableAddress到0x0000000000FFFFFF。
  return MmAllocateContiguousPagesSpecifyCache(uNumberOfPages, pFirstPagePA,
                                               MmCached);
}
