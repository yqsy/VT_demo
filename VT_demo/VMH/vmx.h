...
typedef struct _DEBUG_DR7_ {
  ...
  unsigned RW0 : 2;  //设置DR0访问类型 00B执行断点 01B写断点 10B IO读/写断点11B
                     //读/写断点
...
VOID set_in_cr4(ULONG32 mask);

VOID clear_in_cr4(ULONG32 mask);
...
NTSTATUS VmxSetupVMCS(ULONG_PTR VMM_Stack, PVOID GuestRip, PVOID GuestRsp);

NTSTATUS VmxShutdown(PGUEST_REGS GuestRegs);

NTSTATUS VmxFillGuestSelectorData(PVOID GdtBase, ULONG Segreg, USHORT Selector);
