...
#define MiGetPteAddress(va)                                                    \
  ((PMMPTE)(                                                                   \
      ((((ULONG_PTR)(va)&VIRTUAL_ADDRESS_MASK) >> PTI_SHIFT) << PTE_SHIFT) +   \
      PTE_BASE))
//#define MiGetPteAddress(va) ((PULONG64)(((((ULONG64)(va)&0x0000FFFFFFFFF000)
//>> 12))*8 + PTE_BASE))
#define MiGetPdeAddress(va)                                                    \
  ((PULONG64)(((((ULONG64)(va)&0x0000FFFFFFFFF000) >> 21)) * 8 + PDE_BASE))
#define MiGetPpeAddress(va)                                                    \
  ((PULONG64)(((((ULONG64)(va)&0x0000FFFFFFFFF000) >> 30)) * 8 + PPE_BASE))
#define MiGetPxeAddress(va)                                                    \
  ((PULONG64)(((((ULONG64)(va)&0x0000FFFFFFFFF000) >> 39)) * 8 + PXE_BASE))
...
typedef struct _MMPTE_HARDWARE_PAE {
  ...
  ULONGLONG Valid : 1; // 0 映射状态 该页当前没有映射物理内存=0
                       // 该页当前映射了物理内存=1
  ULONGLONG Write : 1; // 1读写属性 只读=0 可读可写=1
                       // (本位属性只对用户级代码有效 内核态将被忽略
                       // 即总可以读/写)
  ULONGLONG
      Owner : 1; // 2 访问模式 只有内核模式可以访问=0 内核与用户模式均可访问=1
  ULONGLONG WriteThrough : 1; // 3 写入模式 回写模式(写入操作不同步)=0
                              // 直写模式(写入操作同步)=1
  ...
  ULONGLONG LargePage : 1; // 7 分页大小 本位只对PDE有效 PAE模式分为4K和2M两种页
                           // 4K分页=0 2M分页=1
  ULONGLONG Global : 1; // 8 全局页面 该PTE对应到所有进程空间=1
                        // 该PTE对应当前进程空间=0
  ULONGLONG CopyOnWrite : 1;      // 9 写时复制
  ULONGLONG Prototype : 1;        // 10 内存共享 用于R3多进程共享内存
  ULONGLONG reserved0 : 1;        // 11 保留位 未使用11
  ULONGLONG PageFrameNumber : 28; // PFN物理页的页帧号
                                  // 对于PTE将该域和0xfffffffffffff000做与运算
                                  // 可得到该页物理内存基址
