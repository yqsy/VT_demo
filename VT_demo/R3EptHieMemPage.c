#include "H:\备份[重要]\VT_demo\VT_demo\Arch\Intel\EPT.h"
#include "H:\备份[重要]\VT_demo\VT_demo\Arch\Intel\VMX.h"
#include "dbgtool.h"
#include "ntddk.h"
...
PR3EPT_HOOK Page_IsVald(ULONG64 Address, ULONG64 EProcess) {
  ...
  while (Entry != &R3pageList) {
    ...
    if (Address != NULL) {
      ...
      if (TempItem->Data_PAGE_VA == Page &&
          TempItem->TargetProcess == EProcess) {
        DFind = TempItem;
        break;
      }
    }
...
BOOLEAN Page_Add(PR3EPT_HOOK phook) {
  ...
  Temp = (PR3EPT_HOOK)ExAllocatePoolWithTag(NonPagedPool, sizeof(R3EPT_HOOK),
                                            'xrrp');
  if (!Temp) {
    return FALSE;
  }
...
static ULONG64 NTAPI GetGuestPA(PVOID GuestVA) {
  ...
  ULONG64 GuestPA =
      (((PageVA >> 9) & 0x7ffffffff8) + PT_BASE); // MiGetPteAddress(GuestVA);
...
BOOLEAN R3_HideMEM_Violation(IN PGUEST_STATE GuestState) {
  ...
  if (Phook) {
    ...
    if (pViolationData->Fields.Read) {
      ...
      Padd =
          querylist(PmainList, PsGetCurrentProcessId(), PsGetCurrentProcess());
      if (Padd != NULL) {
...
BOOLEAN R3_HideMem(PEPROCESS Process, ULONG64 Address, PVOID Code, ULONG Size) {
  ...
  Phook = (PR3EPT_HOOK)ExAllocatePoolWithTag(NonPagedPool, sizeof(R3EPT_HOOK),
                                             "xrrp");
  ...
  DbgPrint("Gpa:=%p CODEPA:=%p \n", GetGuestPA(Address),
           MmGetPhysicalAddress(CodePage).QuadPart);
  //
  if (IsAdd) {
    if (CreatePage) {
