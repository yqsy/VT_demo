...
typedef NTSTATUS(__fastcall *pfNtDebugActiveProcess)(IN HANDLE ProcessHandle,
                                                     IN HANDLE DebugHandle);

typedef NTSTATUS(__fastcall *pfNtWaitForDebugEvent)(
    IN HANDLE DebugHandle, IN BOOLEAN Alertable,
    IN PLARGE_INTEGER Timeout OPTIONAL, OUT ULONG64 StateChange);

typedef NTSTATUS(__fastcall *pfNtDebugContinue)(IN HANDLE DebugHandle,
                                                IN PCLIENT_ID AppClientId,
                                                IN NTSTATUS ContinueStatus);
typedef NTSTATUS(__fastcall *pfNtRemoveProcessDebug)(IN HANDLE ProcessHandle,
                                                     IN HANDLE DebugHandle);
typedef NTSTATUS(__fastcall *pfDbgkpQueueMessage)(IN PEPROCESS Process,
                                                  IN PETHREAD Thread,
...
typedef VOID(__fastcall *pfDbgkUnMapViewOfSection)(IN PEPROCESS PROCESS,
                                                   IN PVOID BaseAddress);
typedef NTSTATUS(__fastcall *pfDbgkOpenProcessDebugPort)(
    IN PEPROCESS Process, IN KPROCESSOR_MODE PreviousMode,
    OUT HANDLE *DebugHandle);
typedef VOID(__fastcall *pfDbgkCopyProcessDebugPort)(IN PEPROCESS Process,
                                                     IN PEPROCESS Parent,
                                                     IN ULONG64 unknow,
                                                     IN ULONG64 unknow1);
typedef BOOLEAN(__fastcall *pfDbgkForwardException)(
    IN PEXCEPTION_RECORD ExceptionRecord, IN BOOLEAN DebugPort,
