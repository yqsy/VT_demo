...
p_save_handlentry createlist() {
  ...
  p_save_handlentry phead =
      (p_save_handlentry)ExAllocatePool(NonPagedPool, sizeof(_save_handlentry));
  p_save_handlentry ptail = phead;
  ptail->next = NULL;
  p_save_handlentry pnew =
      (p_save_handlentry)ExAllocatePool(NonPagedPool, sizeof(_save_handlentry));
...
// 插入链表
p_save_handlentry insertlist(HANDLE dbgProcessId, PEPROCESS dbgProcessStruct,
                             p_save_handlentry phead) {
...
p_save_handlentry querylist(p_save_handlentry phead, HANDLE dbgProcessId,
