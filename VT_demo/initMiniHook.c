...
VOID __fastcall proxyDbgkUnMapViewOfSection(IN PEPROCESS PROCESS,
                                            IN PVOID BaseAddress);
VOID __fastcall proxyDbgkMapViewOfSection(IN PVOID Section,
                                          IN PVOID BaseAddress,
                                          IN ULONG SectionOffset,
                                          IN ULONG_PTR ViewSize);

NTSTATUS
__fastcall proxyDbgkOpenProcessDebugPort(IN PEPROCESS Process,
                                         IN KPROCESSOR_MODE PreviousMode,
                                         OUT HANDLE *DebugHandle);
////
VOID __fastcall proxyDbgkCopyProcessDebugPort(IN PEPROCESS Process,
                                              IN PEPROCESS Parent);
BOOLEAN
...
DbgkpSetProcessDebugObject_2( //反汇编OK
    IN PEPROCESS Process, IN ULONG64 DebugObject, IN NTSTATUS MsgStatus,
    IN PETHREAD LastThread);
VOID NTAPI proxyDbgkExitProcess(IN NTSTATUS ExitStatus);
VOID NTAPI proxyDbgkExitThread(IN NTSTATUS ExitStatus);
NTSTATUS __fastcall DbgkpQueueMessage_2(IN PEPROCESS Process,
                                        IN PETHREAD Thread,
                                        IN OUT ULONG64 ApiMsg, IN ULONG Flags,
...
VOID installMiniHOOK() {
  ...
  pslp_head_n_byte3 =
      HookKernelApi(DbgkForwardException, (PVOID)proxyDbgkForwardException,
                    &ori_pslp3, &pslp_patch_size3);
...
*/
  ...
  pslp_head_n_byte11 =
      HookKernelApi(DbgkpQueueMessage, (PVOID)DbgkpQueueMessage_2, &ori_pslp11,
                    &pslp_patch_size11);
  /*
...
VOID unMiniHook() {
  ...
  /*
      UnhookKernelApi(DbgkpSetProcessDebugObject, pslp_head_n_byte12,
     pslp_patch_size12);
  */

  UnhookKernelApi(DbgkCopyProcessDebugPort, pslp_head_n_byte2,
                  pslp_patch_size2);

  UnhookKernelApi(DbgkOpenProcessDebugPort, pslp_head_n_byte4,
                  pslp_patch_size4);

  /*
      UnhookKernelApi(DbgkUnMapViewOfSection, pslp_head_n_byte5,
     pslp_patch_size5);

      UnhookKernelApi(DbgkMapViewOfSection, pslp_head_n_byte6,
     pslp_patch_size6);*/
