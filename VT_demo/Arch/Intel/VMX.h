...
enum _VM_EXIT_REASON {
  ...
  EXIT_REASON_APIC_ACCESS = 44, // APIC access. Guest software attempted to
                                // access memory at a physical address on the
                                // APIC-access page.
  EXIT_REASON_VIRTUALIZED_EIO = 45, // EOI virtualization was performed for a
                                    // virtual interrupt whose vector indexed a
                                    // bit set in the EOIexit bitmap
  ...
  EXIT_REASON_EPT_MISCONFIG = 49, // An attempt to access memory with a
                                  // guest-physical address encountered a
...
typedef enum _INV_TYPE {
  ...
  INV_SINGLE_CONTEXT_RETAIN_GLOBALS =
      3 // Invalidate a single VPID context retaining global mappings
} IVVPID_TYPE,
    INVEPT_TYPE;
