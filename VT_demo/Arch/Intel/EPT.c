...
PEPT_MMPTE EptpAllocatePageHighIRQL(IN PEPT_DATA pEPT) {
  ...
  KeBugCheckEx(HYPERVISOR_ERROR, BUG_CHECK_EPT_NO_PAGES, pEPT->Preallocations,
               EPT_PREALLOC_PAGES, 0);
}
...
PEPT_MMPTE EptpAllocatePage(IN PEPT_DATA pEPT) {
  ...
  PEPT_MMPTE ptr = (PEPT_MMPTE)MmAllocateContiguousMemorySpecifyCache(
      PAGE_SIZE, Lowest, Highest, Lowest, MmNonCached);
  ...
  if (ptr) {
    ...
    if (allocEntry) {
      pEntry = ExAllocatePoolWithTag(NonPagedPoolNx, sizeof(EPT_PAGES_ENTRY),
                                     HB_POOL_TAG);
      if (pEntry == NULL) {
        // DPRINT( "HyperBone: CPU %d: %s: Failed to allocate EPT_PAGES_ENTRY
...
NTSTATUS EptUpdateTableRecursive(IN PEPT_DATA pEPTData, IN PEPT_MMPTE pTable,
                                 IN EPT_TABLE_LEVEL level, IN ULONG64 pfn,
                                 IN UCHAR access, IN ULONG64 hostPFN,
                                 IN ULONG count) {
  ...
  return EptUpdateTableRecursive(pEPTData, pNewEPT, level - 1, pfn, access,
                                 hostPFN, count);
}
...
NTSTATUS EptpFillTable(IN PEPT_DATA pEPT, IN PEPT_PML4_ENTRY PML4Ptr) {
  ...
  for (ULONG i = 0; i < g_Data->Memory->NumberOfRuns; i++) {
    ...
    ULONG64 count =
        min(total, EPT_TABLE_ENTRIES - (first & (EPT_TABLE_ENTRIES - 1)));
    ULONG64 hostPFN = first;
    for (ULONG64 pfn = first; total > 0;) {
      if (!NT_SUCCESS(EptUpdateTableRecursive(pEPT, PML4Ptr, EPT_TOP_LEVEL, pfn,
                                              EPT_ACCESS_ALL, hostPFN,
                                              (ULONG)count)))
  ...
  /*for (ULONG64 pfn = 0; pfn <= 0xFEE00; pfn += EPT_TABLE_ENTRIES, hostPFN +=
  EPT_TABLE_ENTRIES)
  {
      if (!NT_SUCCESS( EptUpdateTableRecursive( PML4Ptr, 3, pfn, EPT_ACCESS_ALL,
  hostPFN, EPT_TABLE_ENTRIES ) )) return STATUS_UNSUCCESSFUL;
  }*/
...
NTSTATUS EptFreeIdentityMap(IN PEPT_DATA pEPT) {
  ...
  while (!IsListEmpty(&pEPT->PageList)) {
    ...
    PEPT_PAGES_ENTRY pEntry =
        CONTAINING_RECORD(pListEntry, EPT_PAGES_ENTRY, link);
...
/// <returns>Status code</returns>
NTSTATUS EptGetPTEForPhysical(IN PEPT_PML4_ENTRY PML4, IN PHYSICAL_ADDRESS phys,
...
BOOLEAN R0_And_R3_HideMEM_Violation(IN PGUEST_STATE GuestState) {
  ...
  PEPT_VIOLATION_DATA pViolationData =
      (PEPT_VIOLATION_DATA)&GuestState->ExitQualification;

  PPAGE_HOOK_ENTRY pHookEntry = PHGetHookEntryByPFN(pfn, DATA_PAGE);
  if (pHookEntry) {
    /*DPRINT(
    "HyperBone: CPU %d: %s: Hooked page %s, EIP 0x%p, Linear 0x%p, Physical
    0x%p, Violation data 0x%x\n", CPU_IDX, __FUNCTION__,
    pViolationData->Fields.Execute ? "EXECUTE" : (pViolationData->Fields.Read ?
    "READ" : "WRITE"), GuestState->GuestRip, GuestState->LinearAddress,
    ...
    if (pViolationData->Fields.Read) {
      ...
      __FUNCTION__, GuestState->PhysicalAddress.QuadPart, pViolationData->All
      );*/
    }

    EptUpdateTableRecursive(pEPT, pEPT->PML4Ptr, EPT_TOP_LEVEL, pfn,
                            TargetAccess, TargetPFN, 1);
    EPT_CTX ctx = {0};
    __invept(INV_ALL_CONTEXTS, &ctx);
  ...
  // Create new identity page map
    ...
    /*DPRINT(
    "HyperBone: CPU %d: %s: EPT violation, EIP 0x%p, Linear 0x%p, Physical 0x%p,
...
VOID VmExitEptViolation(IN PGUEST_STATE GuestState) {
  ...
  if (!pViolationData->Fields.PTEWrite && !pViolationData->Fields.PTERead &&
...

  {
    const auto read_failure =
        pViolationData->Fields.Read && !pViolationData->Fields.PTERead;
    const auto write_failure =
        pViolationData->Fields.Write && !pViolationData->Fields.PTEWrite;
    const auto exec_failure =
        pViolationData->Fields.Execute && !pViolationData->Fields.PTEExecute;
...
VOID VmExitEptMisconfig(IN PGUEST_STATE GuestState) {
  ...
  KeBugCheckEx(HYPERVISOR_ERROR, BUG_CHECK_EPT_MISCONFIG,
               GuestState->PhysicalAddress.QuadPart,
