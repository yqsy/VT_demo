...
VOID VmxCheckFeatures() {
  ...
  if (ctl.Fields.ActivateSecondaryControl) {
    ...
    if (ctl2.Fields.EnableEPT != 0) {
      ...
      g_Data->Features.InvSingleAddress =
          vpidcap.Fields.IndividualAddressInvVpid;

      if (vpidcap.Fields.ExecuteOnly == 0)
        DPRINT(
            "HyperBone: CPU %d: %s: No execute-only EPT translation support\n",
...
VOID VmxInjectEvent(INTERRUPT_TYPE InterruptType, VECTOR_EXCEPTION Vector,
                    ULONG WriteLength) {
...
VOID VmxSubvertCPU(IN PVCPU Vcpu) {
  ...
  if (g_Data->Features.SecondaryControls)
    Vcpu->MsrData[VMX_MSR(MSR_IA32_VMX_PROCBASED_CTLS2)].QuadPart =
        __readmsr(MSR_IA32_VMX_PROCBASED_CTLS2);

  // True MSRs, if present
  if (g_Data->Features.TrueMSRs)
    for (ULONG i = VMX_MSR(MSR_IA32_VMX_TRUE_PINBASED_CTLS);
         i <= VMX_MSR(MSR_IA32_VMX_TRUE_ENTRY_CTLS); i++)
      Vcpu->MsrData[i].QuadPart = __readmsr(MSR_IA32_VMX_BASIC + i);

  // VMFUNC, if present
  if (g_Data->Features.VMFUNC)
    Vcpu->MsrData[VMX_MSR(MSR_IA32_VMX_VMFUNC)].QuadPart =
        __readmsr(MSR_IA32_VMX_VMFUNC);
...
VOID VmxpConvertGdtEntry(IN PVOID GdtBase, IN USHORT Selector,
                         OUT PVMX_GDTENTRY64 VmxGdtEntry) {
  ...
  // the "Type" field.
  VmxGdtEntry->Base =
      ((gdtEntry->Bytes.BaseHigh << 24) | (gdtEntry->Bytes.BaseMiddle << 16) |
       (gdtEntry->BaseLow)) &
      MAXULONG;
  VmxGdtEntry->Base |= ((gdtEntry->Bits.Type & 0x10) == 0)
                           ? ((ULONG_PTR)gdtEntry->BaseUpper << 32)
                           : 0;
...
BOOLEAN VmxEnterRoot(IN PVCPU Vcpu) {
  ...
  PIA32_VMX_BASIC_MSR pBasic =
      (PIA32_VMX_BASIC_MSR)&Vcpu->MsrData[VMX_MSR(MSR_IA32_VMX_BASIC)];
...
VOID VmxSetupVMCS(IN PVCPU VpData) {
  ...
  __vmx_vmwrite(
      ...
      VmxpAdjustMsr(VpData->MsrData[VMX_MSR(MSR_IA32_VMX_TRUE_PINBASED_CTLS)],
                    vmPinCtlRequested.All));
  __vmx_vmwrite(
      CPU_BASED_VM_EXEC_CONTROL,
      VmxpAdjustMsr(VpData->MsrData[VMX_MSR(MSR_IA32_VMX_TRUE_PROCBASED_CTLS)],
                    vmCpuCtlRequested.All));
  __vmx_vmwrite(
      SECONDARY_VM_EXEC_CONTROL,
      VmxpAdjustMsr(VpData->MsrData[VMX_MSR(MSR_IA32_VMX_PROCBASED_CTLS2)],
                    vmCpuCtl2Requested.All));
  __vmx_vmwrite(
      VM_EXIT_CONTROLS,
      VmxpAdjustMsr(VpData->MsrData[VMX_MSR(MSR_IA32_VMX_TRUE_EXIT_CTLS)],
                    vmExitCtlRequested.All));
  __vmx_vmwrite(
      VM_ENTRY_CONTROLS,
      VmxpAdjustMsr(VpData->MsrData[VMX_MSR(MSR_IA32_VMX_TRUE_ENTRY_CTLS)],
  ...
  // CS (Ring 0 Code)
  VmxpConvertGdtEntry(state->SpecialRegisters.Gdtr.Base,
                      state->ContextFrame.SegCs, &vmxGdtEntry);
  ...
  // SS (Ring 0 Data)
  VmxpConvertGdtEntry(state->SpecialRegisters.Gdtr.Base,
                      state->ContextFrame.SegSs, &vmxGdtEntry);
  ...
  // DS (Ring 3 Data)
  VmxpConvertGdtEntry(state->SpecialRegisters.Gdtr.Base,
                      state->ContextFrame.SegDs, &vmxGdtEntry);
  ...
  // ES (Ring 3 Data)
  VmxpConvertGdtEntry(state->SpecialRegisters.Gdtr.Base,
                      state->ContextFrame.SegEs, &vmxGdtEntry);
  ...
  VmxpConvertGdtEntry(state->SpecialRegisters.Gdtr.Base,
                      state->ContextFrame.SegFs, &vmxGdtEntry);
  ...
  VmxpConvertGdtEntry(state->SpecialRegisters.Gdtr.Base,
                      state->ContextFrame.SegGs, &vmxGdtEntry);
  ...
  VmxpConvertGdtEntry(state->SpecialRegisters.Gdtr.Base,
                      state->SpecialRegisters.Tr, &vmxGdtEntry);
  ...
  // LDT
  VmxpConvertGdtEntry(state->SpecialRegisters.Gdtr.Base,
                      state->SpecialRegisters.Ldtr, &vmxGdtEntry);
  ...
  __vmx_vmwrite(HOST_RSP, (ULONG_PTR)VpData->VMMStack + KERNEL_STACK_SIZE -
                              sizeof(CONTEXT));
