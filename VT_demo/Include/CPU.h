...
typedef union _IA32_VMX_PROCBASED_CTLS_MSR {
  ...
  struct {
    ...
    ULONG64 ActivateSecondaryControl : 1; // [32 + 31]  Does
                                          // VMX_PROCBASED_CTLS2_MSR exist
  } Fields;
...
typedef union _IA32_VMX_EPT_VPID_CAP_MSR {
  ULONG64 All;
  struct {
    ULONG64 ExecuteOnly : 1; // Bit 0 defines if the EPT implementation supports
                             // execute-only translation
    ULONG64 Reserved1 : 31;  // Undefined
    ULONG64 Reserved2 : 8;   // Undefined
    ULONG64 IndividualAddressInvVpid : 1; // Bit 40 defines if type 0 INVVPID
