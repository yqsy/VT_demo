...
#define PAGES_PER_ENTRY                                                        \
  ((PAGE_SIZE - sizeof(LIST_ENTRY) - sizeof(ULONG64)) /                        \
   sizeof(union _EPT_MMPTE *))
#define EPT_PREALLOC_PAGES 512

/// <summary>
/// CPU vendor
/// </summary>
typedef enum _CPU_VENDOR { CPU_Other = 0, CPU_Intel, CPU_AMD } CPU_VENDOR;
...
typedef struct _VMX_FEATURES {
  ...
  ULONG64
      ExecOnlyEPT : 1; // EPT translation with execute-only access is supported
