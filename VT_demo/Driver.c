...
NTSTATUS HOOKIDT(ULONG IDTID, PVOID NewfcuncAddress, __in BOOLEAN userDPL,
                 __out PVOID *oldTRAP1);
...
PGLOBAL_DATA AllocGlobalData() {
  ...
  ULONG_PTR size =
      FIELD_OFFSET(GLOBAL_DATA, cpu_data) + cpu_count * sizeof(VCPU);
  PGLOBAL_DATA pData =
      (PGLOBAL_DATA)ExAllocatePoolWithTag(NonPagedPoolNx, size, HB_POOL_TAG);
  if (pData == NULL)
    return NULL;
  ...
  pData->MSRBitmap =
      ExAllocatePoolWithTag(NonPagedPoolNx, PAGE_SIZE, HB_POOL_TAG);
  ...
  for (ULONG i = 0; i < cpu_count; i++) {
    ...
    for (ULONG j = 0; j < EPT_PREALLOC_PAGES; j++) {
      Vcpu->EPT.Pages[j] = MmAllocateContiguousMemorySpecifyCache(
          PAGE_SIZE, low, high, low, MmNonCached);
      if (Vcpu->EPT.Pages[j] != NULL) {
        UtilProtectNonpagedMemory(Vcpu->EPT.Pages[j], PAGE_SIZE,
                                  PAGE_READWRITE);
...
BOOLEAN CheckIOCTLData() {
  if (KiRetireDpcList == NULL || ObDuplicateObject == NULL ||
      KiSaveDebugRegisterState == NULL || KiUmsCallEntry == NULL ||
      KiSystemServiceExit == NULL || KeGdiFlushUserBatch == NULL ||
      KiSystemServiceRepeat == NULL || PspSystemDlls == NULL) {
    return FALSE;
  }

  if (KeServiceDescriptorTable == NULL ||
      KeServiceDescriptorTableShadow == NULL ||
      KiSystemServiceCopyEnd == NULL || DbgkDebugObjectType == NULL) {
    return FALSE;
  }
  if (DbgkCopyProcessDebugPort == NULL || KiDispatchException == NULL ||
      DbgkForwardException == NULL || LpcRequestWaitReplyPortEx == NULL) {

    return FALSE;
  }
  if (KiDispatchException == NULL || MmGetFileNameForSection == NULL ||
      PsGetNextProcess == NULL || PsTerminateProcess == NULL ||
      DbgkOpenProcessDebugPort == NULL || DbgkUnMapViewOfSection == NULL ||
      DbgkMapViewOfSection == NULL) {
    return FALSE;
  }
  if (ExGetCallBackBlockRoutine == NULL ||
      ObpCallPreOperationCallbacks == NULL || DbgkExitProcess == NULL ||
      DbgkExitThread == NULL || DbgkpSendApiMessage == NULL ||
      DbgkpQueueMessage == NULL || MmGetFileNameForAddress == NULL) {
    return FALSE;
  }
  if (NtQueryInformationThread == NULL || ExCompareExchangeCallBack == NULL ||
      RtlpCopyLegacyContextX86 == NULL) {
    return FALSE;
  }

  if (NtReadVirtualMemory == NULL || NtWriteVirtualMemory == NULL ||
      XNtOpenProcess == NULL || KiAttachProcess == NULL) {
    return FALSE;
  }
...
NTSTATUS DispatchIoctl(PDEVICE_OBJECT pDevObj, PIRP pIrp) {
  ...
  case IOCTL_dbgProcessId: {
    ...
    insertlist(PsGetCurrentProcessId(), PsGetCurrentProcess(),
               PmainList); //加入DBG进程
    if (VtMode) {
      ///暂时没处理这个位置
    } else {
  ...
  case IOCTL_RtlpCopyLegacyContextX86: {
    ...
    memcpy(&RtlpCopyLegacyContextX86, pIoBuffer,
           sizeof(RtlpCopyLegacyContextX86));
  ...
  case IOCTL_SwapContext_PatchXRstor: {
    ...
    memcpy(&SwapContext_PatchXRstor, pIoBuffer,
           sizeof(SwapContext_PatchXRstor));
  ...
  case IOCTL_DbgkpPostFakeProcessCreateMessages: {
    ...
    memcpy(&DbgkpPostFakeProcessCreateMessages, pIoBuffer,
           sizeof(DbgkpPostFakeProcessCreateMessages));
  ...
  case IOCTL_DbgkpSetProcessDebugObject: {
    ...
    memcpy(&DbgkpSetProcessDebugObject, pIoBuffer,
           sizeof(DbgkpSetProcessDebugObject));
  ...
  case IOCTL_ExCompareExchangeCallBack: {
    ...
    memcpy(&ExCompareExchangeCallBack, pIoBuffer,
           sizeof(ExCompareExchangeCallBack));
  ...
  case IOCTL_NtQueryInformationThread: {
    ...
    memcpy(&NtQueryInformationThread, pIoBuffer,
           sizeof(NtQueryInformationThread));
  ...
  case IOCTL_ObpCallPreOperationCallbacks: {
    ...
    memcpy(&ObpCallPreOperationCallbacks, pIoBuffer,
           sizeof(ObpCallPreOperationCallbacks));
  ...
  case IOCTL_MmGetFileNameForAddress: {
    ...
    memcpy(&MmGetFileNameForAddress, pIoBuffer,
           sizeof(MmGetFileNameForAddress));
  ...
  case IOCTL_ExGetCallBackBlockRoutine: {
    ...
    memcpy(&ExGetCallBackBlockRoutine, pIoBuffer,
           sizeof(ExGetCallBackBlockRoutine));
  ...
  case IOCTL_DbgkOpenProcessDebugPort: {
    ...
    memcpy(&DbgkOpenProcessDebugPort, pIoBuffer,
           sizeof(DbgkOpenProcessDebugPort));
  ...
  case IOCTL_MmGetFileNameForSection: {
    ...
    memcpy(&MmGetFileNameForSection, pIoBuffer,
           sizeof(MmGetFileNameForSection));
  ...
  case IOCTL_DbgkpProcessDebugPortMutex: {
    ...
    memcpy(&DbgkpProcessDebugPortMutex, pIoBuffer,
           sizeof(DbgkpProcessDebugPortMutex));
  ...
  case IOCTL_DbgkCopyProcessDebugPort: {
    ...
    memcpy(&DbgkCopyProcessDebugPort, pIoBuffer,
           sizeof(DbgkCopyProcessDebugPort));
  ...
  case IOCTL_KiSaveDebugRegisterState: {
    ...
    memcpy(&KiSaveDebugRegisterState, pIoBuffer,
           sizeof(KiSaveDebugRegisterState));
  ...
  case IOCTL_KiRestoreDebugRegisterState: {
    ...
    memcpy(&KiRestoreDebugRegisterState, pIoBuffer,
           sizeof(KiRestoreDebugRegisterState));
  ...
  case IOCTL_KeServiceDescriptorTable: {
    ...
    memcpy(&KeServiceDescriptorTable, pIoBuffer,
  ...
  case IOCTL_DbgkpPostFakeThreadMessages: {
    ...
    memcpy(&DbgkpPostFakeThreadMessages, pIoBuffer,
           sizeof(DbgkpPostFakeThreadMessages));
  ...
  case IOCTL_DbgkpPostModuleMessages: {
    ...
    memcpy(&DbgkpPostModuleMessages, pIoBuffer,
           sizeof(DbgkpPostModuleMessages));
  ...
  case IOCTL_KeServiceDescriptorTableShadow: {
    ...
    memcpy(&KeServiceDescriptorTableShadow, pIoBuffer,
...
VOID NTAPI RegSetCr3(PVOID NewCr3) { __writecr3((ULONG64)NewCr3); }
//切入客户机进程空间
...
NTSTATUS LoadHV() {
  ...
  if (UtilSSDTEntry(0) == 0) {
    DPRINT("HyperBone: CPU %d: %s: Failed to Get SSDT/Kernel base, can't "
           "continue\n",
           CPU_IDX, __FUNCTION__);
