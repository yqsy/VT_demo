...
PVOID HookKernelApi2(IN PVOID ApiAddress, IN PVOID Proxy_ApiAddress,
...
//传入：待HOOK函数地址，代理函数地址，接收原始函数地址的指针，接收补丁长度的指针；返回：原来头N字节的数据
PVOID HookKernelApi1(IN PVOID ApiAddress, IN PVOID Proxy_ApiAddress,
                     OUT PVOID *Original_ApiAddress, OUT ULONG *PatchSize) {
  KIRQL irql;
  UINT64 tmpv;
  PVOID head_n_byte, ori_func;
...
VOID UnhookKernelApi2(IN PVOID ApiAddress, IN PVOID OriCode,
                      IN ULONG PatchSize) {
  KIRQL irql;
  // irql=WPOFFx64();
...
PVOID HookKernelApi4_6bit(IN ULONG64 ApiAddress, IN PVOID Proxy_ApiAddress,
  ...
  if (KrnlBase == 0 || KrnlSize == 0 || structb->orgcode == NULL ||
      structb->orgcodenumber == NULL)
    return;
