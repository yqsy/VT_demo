...
{
  /*0x000*/ struct _KERNEL_STACK_SEGMENT
      Current; // 5 elements, 0x28 bytes (sizeof)
  /*0x028*/ struct _KERNEL_STACK_SEGMENT
      Previous; // 5 elements, 0x28 bytes (sizeof)
...
#define DEBUG_EVENT_INACTIVE                                                   \
  (0x04) // The message is in inactive. It may be activated or deleted later
#define DEBUG_EVENT_RELEASE (0x08) // Release rundown protection on this thread
#define DEBUG_EVENT_PROTECT_FAILED                                             \
  (0x10) // Rundown protection failed to be acquired on this thread
...
#define PS_PROCESS_FLAGS_CREATE_REPORTED                                       \
  0x00000001UL // Create process debug call has occurred
#define PS_PROCESS_FLAGS_NO_DEBUG_INHERIT                                      \
  0x00000002UL // Don't inherit debug port
#define PS_PROCESS_FLAGS_PROCESS_EXITING 0x00000004UL // PspExitProcess entered
#define PS_PROCESS_FLAGS_PROCESS_DELETE                                        \
  0x00000008UL // Delete process has been issued
...
#define PS_PROCESS_FLAGS_WOW64_4GB_VA_SPACE                                    \
  0x00000200UL // Wow64 process with 4gb virtual address space
...
#define PS_PROCESS_FLAGS_SET_TIMER_RESOLUTION                                  \
  0x00001000UL // SetTimerResolution has been called
#define PS_PROCESS_FLAGS_BREAK_ON_TERMINATION                                  \
  0x00002000UL // Break on process termination
#define PS_PROCESS_FLAGS_CREATING_SESSION                                      \
  0x00004000UL // Process is creating a session
#define PS_PROCESS_FLAGS_USING_WRITE_WATCH                                     \
  0x00008000UL // Process is using the write watch APIs
#define PS_PROCESS_FLAGS_IN_SESSION 0x00010000UL // Process is in a session
#define PS_PROCESS_FLAGS_OVERRIDE_ADDRESS_SPACE                                \
  0x00020000UL // Process must use native address space (Win64 only)
#define PS_PROCESS_FLAGS_HAS_ADDRESS_SPACE                                     \
  0x00040000UL // This process has an address space
#define PS_PROCESS_FLAGS_LAUNCH_PREFETCHED                                     \
  0x00080000UL // Process launch was prefetched
#define PS_PROCESS_INJECT_INPAGE_ERRORS                                        \
  0x00100000UL // Process should be given inpage errors - hardcoded in trap.asm
               // too
#define PS_PROCESS_FLAGS_VM_TOP_DOWN                                           \
  0x00200000UL // Process memory allocations default to top-down
#define PS_PROCESS_FLAGS_IMAGE_NOTIFY_DONE                                     \
  0x00400000UL // We have sent a message for this image
#define PS_PROCESS_FLAGS_PDE_UPDATE_NEEDED                                     \
  0x00800000UL // The system PDEs need updating for this process (NT32 only)
#define PS_PROCESS_FLAGS_VDM_ALLOWED                                           \
  0x01000000UL // Process allowed to invoke NTVDM support
#define PS_PROCESS_FLAGS_SMAP_ALLOWED                                          \
  0x02000000UL // Process allowed to invoke SMAP support
#define PS_PROCESS_FLAGS_CREATE_FAILED 0x04000000UL // Process create failed

#define PS_PROCESS_FLAGS_DEFAULT_IO_PRIORITY                                   \
  0x38000000UL // The default I/O priority for created threads. (3 bits)
...
typedef struct _OBJECT_TYPE_INITIALIZER // 25 elements, 0x70 bytes (sizeof)
{
  ...
  /*0x00C*/ struct _GENERIC_MAPPING
      GenericMapping; // 4 elements, 0x10 bytes (sizeof)
...
typedef struct _OBJECT_TYPE_S // 12 elements, 0xD0 bytes (sizeof)
{
  ...
  /*0x040*/ struct _OBJECT_TYPE_INITIALIZER
      TypeInfo;                            // 25 elements, 0x70 bytes (sizeof)
...
typedef NTSTATUS (*OBCREATEOBJECTTYPE)(PUNICODE_STRING usTypeName,
  ...
  struct {
    ...
    union {
      ...
      struct {
        ...
        ULONG LongMode : 1;
        ULONG DefaultBig : 1; //即INTEL的D/B (default operation size/default
...
{
  /*0x000*/ struct _LIST_ENTRY
      SortedListEntry; // 2 elements, 0x10 bytes (sizeof)
  /*0x010*/ struct _LIST_ENTRY
      IdleOnlyListHead; // 2 elements, 0x10 bytes (sizeof)
...
{
  /*0x000*/ struct _MMADDRESS_NODE
      BalancedRoot; // 5 elements, 0x28 bytes (sizeof)
...
typedef struct _EJOB // 42 elements, 0x1C8 bytes (sizeof)
{
  ...
  /*0x028*/ struct _LIST_ENTRY
      ProcessListHead;                 // 2 elements, 0x10 bytes (sizeof)
  /*0x038*/ struct _ERESOURCE JobLock; // 15 elements, 0x68 bytes (sizeof)
  /*0x0A0*/ union _LARGE_INTEGER
      TotalUserTime; // 4 elements, 0x8 bytes (sizeof)
  /*0x0A8*/ union _LARGE_INTEGER
      TotalKernelTime; // 4 elements, 0x8 bytes (sizeof)
  /*0x0B0*/ union _LARGE_INTEGER
      ThisPeriodTotalUserTime; // 4 elements, 0x8 bytes (sizeof)
  /*0x0B8*/ union _LARGE_INTEGER
      ThisPeriodTotalKernelTime; // 4 elements, 0x8 bytes (sizeof)
  ...
  /*0x0D0*/ union _LARGE_INTEGER
      PerProcessUserTimeLimit; // 4 elements, 0x8 bytes (sizeof)
  /*0x0D8*/ union _LARGE_INTEGER
      PerJobUserTimeLimit; // 4 elements, 0x8 bytes (sizeof)
  ...
  /*0x1A8*/ struct _EX_PUSH_LOCK
      MemoryLimitsLock;                     // 7 elements, 0x8 bytes (sizeof)
...
{
  /*0x000*/ struct _EX_PUSH_LOCK
      WorkingSetMutex; // 7 elements, 0x8 bytes (sizeof)
  /*0x008*/ struct _KGATE *ExitGate;
  /*0x010*/ VOID *AccessLog;
  /*0x018*/ struct _LIST_ENTRY
      WorkingSetExpansionLinks; // 2 elements, 0x10 bytes (sizeof)
...
{
  /*0x000*/ struct _DISPATCHER_HEADER
      Header; // 29 elements, 0x18 bytes (sizeof)
  /*0x018*/ struct _LIST_ENTRY
      ProfileListHead; // 2 elements, 0x10 bytes (sizeof)
  /*0x028*/ UINT64 DirectoryTableBase;
  /*0x030*/ struct _LIST_ENTRY
      ThreadListHead; // 2 elements, 0x10 bytes (sizeof)
  ...
  /*0x080*/ struct _SINGLE_LIST_ENTRY
      SwapListEntry; // 1 elements, 0x8 bytes (sizeof)
  /*0x088*/ struct _KAFFINITY_EX
      ActiveProcessors; // 4 elements, 0x28 bytes (sizeof)
  union                 // 2 elements, 0x4 bytes (sizeof)
  {
  ...
  /*0x0E0*/ struct _LIST_ENTRY
      ProcessListEntry; // 2 elements, 0x10 bytes (sizeof)
  /*0x0F0*/ UINT64 CycleTime;
  /*0x0F8*/ ULONG32 KernelTime;
  /*0x0FC*/ ULONG32 UserTime;
  /*0x100*/ ULONG64 *InstrumentationCallback;
  /*0x108*/ union _KGDTENTRY64
      LdtSystemDescriptor; // 7 elements, 0x10 bytes (sizeof)
  /*0x118*/ ULONG64 *LdtBaseAddress;

  /*0x120*/ struct _KGUARDED_MUTEX64
      LdtProcessLock; // 7 elements, 0x38 bytes (sizeof)
...
typedef struct _RTL_UMS_CONTEXT // 28 elements, 0x540 bytes (sizeof)
{
  ...
  /*0x508*/ struct _GROUP_AFFINITY
      AffinityMask; // 3 elements, 0x10 bytes (sizeof)
...
typedef struct _UMS_CONTROL_BLOCK // 22 elements, 0x98 bytes (sizeof)
{
  ...
  union // 2 elements, 0x6C bytes (sizeof)
  {
    struct // 6 elements, 0x6C bytes (sizeof)
    {
      ...
      /*0x060*/ struct _LIST_ENTRY
          QueueEntry; // 2 elements, 0x10 bytes (sizeof)
...
{
  /*0x000*/ struct _KSPECIAL_REGISTERS
      SpecialRegisters; // 27 elements, 0xD8 bytes (sizeof)
...
{
  /*0x000*/ union _SLIST_HEADER
      PagedPoolSListHead; // 5 elements, 0x10 bytes (sizeof)
  ...
  /*0x078*/ ULONG32 Right;
  /*0x07C*/ ULONG32 Left;
  /*0x080*/ struct _CACHED_KSTACK_LIST
      CachedKernelStacks; // 5 elements, 0x20 bytes (sizeof)
...
{
  /*0x000*/ struct _KAFFINITY_EX
      DomainMembers; // 4 elements, 0x28 bytes (sizeof)
...
typedef struct _PPM_IDLE_STATES // 8 elements, 0xA0 bytes (sizeof)
{
  ...
  /*0x018*/ struct _KAFFINITY_EX
      TargetProcessors; // 4 elements, 0x28 bytes (sizeof)
...
typedef struct _PPM_FFH_THROTTLE_STATE_INFO // 5 elements, 0x20 bytes (sizeof)
{
  ...
  /*0x018*/ union _LARGE_INTEGER
      LastLogTickCount; // 4 elements, 0x8 bytes (sizeof)
...
typedef struct _PROCESSOR_POWER_STATE // 27 elements, 0x100 bytes (sizeof)
{
  ...
  /*0x044*/ UINT8 _PADDING0_[0x4];
  /*0x048*/ struct _PPM_FFH_THROTTLE_STATE_INFO
      FFHThrottleStateInfo;             // 5 elements, 0x20 bytes (sizeof)
...
typedef struct _REQUEST_MAILBOX // 3 elements, 0x40 bytes (sizeof)
{
  ...
  /*0x010*/ struct _KREQUEST_PACKET
      RequestPacket; // 2 elements, 0x20 bytes (sizeof)
...
typedef struct _KPRCB // 242 elements, 0x4D00 bytes (sizeof)
{
  ...
  /*0x040*/ struct _KPROCESSOR_STATE
      ProcessorState; // 2 elements, 0x5B0 bytes (sizeof)
  /*0x5F0*/ CHAR CpuType;
  /*0x5F1*/ CHAR CpuID;
  ...
  /*0x2088*/ struct _SINGLE_LIST_ENTRY
      DeferredReadyListHead; // 1 elements, 0x8 bytes (sizeof)
  ...
  /*0x20E8*/ union _LARGE_INTEGER
      IoReadTransferCount; // 4 elements, 0x8 bytes (sizeof)
  /*0x20F0*/ union _LARGE_INTEGER
      IoWriteTransferCount; // 4 elements, 0x8 bytes (sizeof)
  /*0x20F8*/ union _LARGE_INTEGER
      IoOtherTransferCount; // 4 elements, 0x8 bytes (sizeof)
  ...
  /*0x2200*/ struct _KTIMER_TABLE
      TimerTable;                   // 2 elements, 0x2200 bytes (sizeof)
  ...
  /*0x4800*/ struct _PROCESSOR_POWER_STATE
      PowerState; // 27 elements, 0x100 bytes (sizeof)
  ...
  /*0x4A00*/ union _SLIST_HEADER
      InterruptObjectPool; // 5 elements, 0x10 bytes (sizeof)
  /*0x4A10*/ union _SLIST_HEADER
      HypercallPageList; // 5 elements, 0x10 bytes (sizeof)
  ...
  /*0x4A68*/ struct _KAFFINITY_EX
      PackageProcessorSet; // 4 elements, 0x28 bytes (sizeof)
  ...
  /*0x4BD0*/ union _LARGE_INTEGER
      UpdateSignature; // 4 elements, 0x8 bytes (sizeof)
...
typedef struct _THREAD_PERFORMANCE_DATA // 10 elements, 0x1C0 bytes (sizeof)
{
  ...
  /*0x004*/ struct _PROCESSOR_NUMBER
      ProcessorNumber; // 3 elements, 0x4 bytes (sizeof)
  ...
  /*0x028*/ struct _COUNTER_READING
      CycleTime; // 4 elements, 0x18 bytes (sizeof)
...
typedef struct _KTHREAD // 126 elements, 0x360 bytes (sizeof)
{
  ...
  union // 2 elements, 0x10 bytes (sizeof)
  {
    /*0x0A0*/ struct _LIST_ENTRY
        WaitListEntry; // 2 elements, 0x10 bytes (sizeof)
    /*0x0A0*/ struct _SINGLE_LIST_ENTRY
        SwapListEntry; // 1 elements, 0x8 bytes (sizeof)
  };
  ...
  /*0x1C8*/ struct _LIST_ENTRY
      QueueListEntry; // 2 elements, 0x10 bytes (sizeof)
  ...
  /*0x200*/ struct _GROUP_AFFINITY
      UserAffinity; // 3 elements, 0x10 bytes (sizeof)
  ...
  union // 2 elements, 0x30 bytes (sizeof)
  {
    /*0x240*/ struct _KAPC_STATE
        SavedApcState; // 5 elements, 0x30 bytes (sizeof)
  ...
  {
    /*0x2D8*/ struct _KSEMAPHORE
        SuspendSemaphore; // 2 elements, 0x20 bytes (sizeof)
  ...
  /*0x2F8*/ struct _LIST_ENTRY
      ThreadListEntry; // 2 elements, 0x10 bytes (sizeof)
  /*0x308*/ struct _LIST_ENTRY
      MutantListHead; // 2 elements, 0x10 bytes (sizeof)
...
typedef struct _ETHREAD // 88 elements, 0x498 bytes (sizeof)
{
  ...
  union                                      // 2 elements, 0x10 bytes (sizeof)
  {
    ...
    /*0x368*/ struct _LIST_ENTRY
        KeyedWaitChain; // 2 elements, 0x10 bytes (sizeof)
  };
  /*0x378*/ LONG32 ExitStatus;
  /*0x37C*/ UINT8 _PADDING0_[0x4];
  union // 2 elements, 0x10 bytes (sizeof)
  {
    /*0x380*/ struct _LIST_ENTRY
        PostBlockList; // 2 elements, 0x10 bytes (sizeof)
  ...
  /*0x3A0*/ struct _LIST_ENTRY
      ActiveTimerListHead;         // 2 elements, 0x10 bytes (sizeof)
                                   // ULONG64 unknow;
  /*0x3B0*/ struct _CLIENT_ID Cid; // 2 elements, 0x10 bytes (sizeof)
  union                            // 2 elements, 0x20 bytes (sizeof)
  {
    /*0x3C0*/ struct _KSEMAPHORE
        KeyedWaitSemaphore; // 2 elements, 0x20 bytes (sizeof)
    /*0x3C0*/ struct _KSEMAPHORE
        AlpcWaitSemaphore; // 2 elements, 0x20 bytes (sizeof)
  };
  ...
  /*0x420*/ struct _LIST_ENTRY
      ThreadListEntry; // 2 elements, 0x10 bytes (sizeof)
  /*0x430*/ struct _EX_RUNDOWN_REF
      RundownProtect;                        // 2 elements, 0x8 bytes (sizeof)
  ...
  /*0x468*/ struct _LIST_ENTRY
      AlpcWaitListEntry; // 2 elements, 0x10 bytes (sizeof)
  /*0x478*/ ULONG32 CacheManagerCount;
  /*0x47C*/ ULONG32 IoBoostCount;
  /*0x480*/ UINT64 IrpListLock;
  /*0x488*/ VOID *ReservedForSynchTracking;
  /*0x490*/ struct _SINGLE_LIST_ENTRY
      CmCallbackListHead; // 1 elements, 0x8 bytes (sizeof)
...
typedef struct _PEB // 91 elements, 0x380 bytes (sizeof)
{
  ...
  /*0x0C0*/ union _LARGE_INTEGER
      CriticalSectionTimeout; // 4 elements, 0x8 bytes (sizeof)
  ...
  /*0x2C8*/ union _ULARGE_INTEGER
      AppCompatFlags; // 4 elements, 0x8 bytes (sizeof)
  /*0x2D0*/ union _ULARGE_INTEGER
      AppCompatFlagsUser; // 4 elements, 0x8 bytes (sizeof)
  ...
  /*0x328*/ struct _LIST_ENTRY64_S
      FlsListHead; // 2 elements, 0x10 bytes (sizeof)
...
typedef struct _EPROCESS_S // 135 elements, 0x4D0 bytes (sizeof)
{
  ...
  /*0x178*/ struct _EX_RUNDOWN_REF
      RundownProtect; // 2 elements, 0x8 bytes (sizeof)
  /*0x180*/ VOID *UniqueProcessId;
  /*0x188*/ struct _LIST_ENTRY
      ActiveProcessLinks; // 2 elements, 0x10 bytes (sizeof)
  ...
  /*0x1E0*/ struct _LIST_ENTRY
      SessionProcessLinks; // 2 elements, 0x10 bytes (sizeof)
  ...
  /*0x218*/ struct _EX_PUSH_LOCK
      AddressCreationLock; // 7 elements, 0x8 bytes (sizeof)
  ...
  union // 2 elements, 0x8 bytes (sizeof)
  {
    /*0x2D0*/ struct _HARDWARE_PTE
        PageDirectoryPte; // 16 elements, 0x8 bytes (sizeof)
    /*0x2D0*/ UINT64 Filler;
  };
  ...
  /*0x348*/ union _LARGE_INTEGER
      ReadOperationCount; // 4 elements, 0x8 bytes (sizeof)
  /*0x350*/ union _LARGE_INTEGER
      WriteOperationCount; // 4 elements, 0x8 bytes (sizeof)
  /*0x358*/ union _LARGE_INTEGER
      OtherOperationCount; // 4 elements, 0x8 bytes (sizeof)
  /*0x360*/ union _LARGE_INTEGER
      ReadTransferCount; // 4 elements, 0x8 bytes (sizeof)
  /*0x368*/ union _LARGE_INTEGER
      WriteTransferCount; // 4 elements, 0x8 bytes (sizeof)
  /*0x370*/ union _LARGE_INTEGER
      OtherTransferCount; // 4 elements, 0x8 bytes (sizeof)
  /*0x378*/ UINT64 CommitChargeLimit;
  /*0x380*/ UINT64 CommitChargePeak;
  /*0x388*/ VOID *AweInfo;
  /*0x390*/ struct _SE_AUDIT_PROCESS_CREATION_INFO
      SeAuditProcessCreationInfo; // 1 elements, 0x8 bytes (sizeof)
  /*0x398*/ struct _MMSUPPORT Vm; // 21 elements, 0x88 bytes (sizeof)
  /*0x420*/ struct _LIST_ENTRY
      MmProcessLinks; // 2 elements, 0x10 bytes (sizeof)
  ...
  /*0x488*/ struct _ALPC_PROCESS_CONTEXT
      AlpcContext; // 3 elements, 0x20 bytes (sizeof)
  /*0x4A8*/ struct _LIST_ENTRY
      TimerResolutionLink; // 2 elements, 0x10 bytes (sizeof)
...
typedef struct _PEB_LDR_DATA // 9 elements, 0x58 bytes (sizeof)
{
  ...
  /*0x008*/ VOID *SsHandle;
  /*0x010*/ struct _LIST_ENTRY
      InLoadOrderModuleList; // 2 elements, 0x10 bytes (sizeof)
  /*0x020*/ struct _LIST_ENTRY
      InMemoryOrderModuleList; // 2 elements, 0x10 bytes (sizeof)
  /*0x030*/ struct _LIST_ENTRY
      InInitializationOrderModuleList; // 2 elements, 0x10 bytes (sizeof)
...
{
  /*0x000*/ struct _LIST_ENTRY
      InLoadOrderLinks; // 2 elements, 0x10 bytes (sizeof)
  /*0x010*/ struct _LIST_ENTRY
      InMemoryOrderLinks; // 2 elements, 0x10 bytes (sizeof)
  /*0x020*/ struct _LIST_ENTRY
      InInitializationOrderLinks; // 2 elements, 0x10 bytes (sizeof)
  /*0x030*/ VOID *DllBase;
  /*0x038*/ VOID *EntryPoint;
  /*0x040*/ ULONG32 SizeOfImage;
  /*0x044*/ UINT8 _PADDING0_[0x4];
  /*0x048*/ struct _UNICODE_STRING
      FullDllName; // 3 elements, 0x10 bytes (sizeof)
  /*0x058*/ struct _UNICODE_STRING
      BaseDllName; // 3 elements, 0x10 bytes (sizeof)
  /*0x068*/ ULONG32 Flags;
  ...
  /*0x098*/ struct _LIST_ENTRY
      ForwarderLinks; // 2 elements, 0x10 bytes (sizeof)
  /*0x0A8*/ struct _LIST_ENTRY
      ServiceTagLinks;                      // 2 elements, 0x10 bytes (sizeof)
...
typedef struct _TEB64 // 101 elements, 0x1818 bytes (sizeof)
{
  ...
  /*0x2F0*/ struct _GDI_TEB_BATCH64
      GdiTebBatch; // 3 elements, 0x4E8 bytes (sizeof)
  ...
  /*0x1258*/ struct _STRING64
      StaticUnicodeString; // 3 elements, 0x10 bytes (sizeof)
  ...
  union // 3 elements, 0x4 bytes (sizeof)
  {
    /*0x1744*/ struct _PROCESSOR_NUMBER
        CurrentIdealProcessor; // 3 elements, 0x4 bytes (sizeof)
