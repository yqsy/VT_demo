...
NTSTATUS UtilQueryPhysicalMemory() {
  ...
  for (PPHYSICAL_MEMORY_RANGE pRange = pBaseRange;
       pRange->NumberOfBytes.QuadPart != 0; pRange++) {
  ...
  runsCount += 2;
  ULONG size = sizeof(PPHYSICAL_MEMORY_DESCRIPTOR) +
               runsCount * sizeof(PHYSICAL_MEMORY_RUN);
  ...
  if (g_Data->Memory != NULL) {
    ...
    runsCount = 0;
    for (PPHYSICAL_MEMORY_RANGE pRange = pBaseRange;
         pRange->BaseAddress.QuadPart != 0; pRange++, runsCount++) {
      g_Data->Memory->Run[runsCount].BasePage =
          PFN(pRange->BaseAddress.QuadPart);
      g_Data->Memory->Run[runsCount].PageCount =
          PFN(pRange->NumberOfBytes.QuadPart);
    }
