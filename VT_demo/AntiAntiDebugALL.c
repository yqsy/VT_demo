...
typedef ULONG(__fastcall *pfNtOpenProcess)(__out PHANDLE ProcessHandle,
                                           __in ACCESS_MASK AccessMask,
                                           __in PVOID ObjectAttributes,
                                           __in PCLIENT_ID ClientId);
typedef signed __int64(__fastcall *pfNtReadVirtualMemory)(
    PHANDLE ProcessHandle, unsigned __int64 AccessMode, __int64 a3, __int64 a4,
    unsigned __int64 a5);
typedef signed __int64(__fastcall *pfNtWriteVirtualMemory)(
    PHANDLE ProcessHandle, __int64 a2, unsigned __int64 AccessMode, __int64 a4,
    unsigned __int64 a5);
...
typedef NTSTATUS(__fastcall *pfKiAttachProcess)(IN PKTHREAD Thread,
...
void GetKernelModuleBase(char *lpModuleName, ULONG64 *ByRefBase,
                         ULONG *ByRefSize);
...
PVOID HookKernelApi(IN PVOID ApiAddress, IN PVOID Proxy_ApiAddress,
...
PVOID HookKernelApi4_6bit(IN ULONG64 ApiAddress, IN PVOID Proxy_ApiAddress,
                          PHOOK_64BIT structb);
...
typedef __int64(__fastcall *pfObpCallPreOperationCallbacks)(__int64 a1,
                                                            __int64 a2,
                                                            __int64 a3);
typedef __int64(__fastcall *pfExGetCallBackBlockRoutine)(__int64 a1);
typedef BOOLEAN(__fastcall *pfExCompareExchangeCallBack)(ULONG64 a1, ULONG64 a2,
                                                         ULONG64 a3);
typedef NTSTATUS(__fastcall *pfNtQueryInformationThread)(
...
typedef(__fastcall *pfnNtReadVirtualMemory)(IN HANDLE ProcessHandle,
                                            IN PVOID BaseAddress,
                                            OUT PVOID Buffer,
                                            IN ULONG BufferLength,
                                            OUT PULONG ReturnLength OPTIONAL);
typedef(__fastcall *pfnNtWriteVirtualMemory)(IN HANDLE ProcessHandle,
                                             IN PVOID BaseAddress,
                                             OUT PVOID Buffer,
                                             IN ULONG BufferLength,
                                             OUT PULONG ReturnLength OPTIONAL);
typedef NTSTATUS(__fastcall *pfNtSetInformationThread)(
...
ULONG KrnlSize = 0;
HOOK_64BIT orgcode;
typedef NTSTATUS(__fastcall *pfPsLookupProcessByProcessId)(
    _In_ HANDLE ProcessId, _Out_ PEPROCESS *Process);
typedef NTSTATUS(__fastcall *pfPsLookupThreadByThreadId)(
...
__int64 __fastcall proxyKiAttachProcess(IN PKTHREAD Thread,
                                        IN PKPROCESS Process,
                                        IN PKLOCK_QUEUE_HANDLE ApcLock,
                                        IN PRKAPC_STATE SavedApcState) {
  PEPROCESS g_Guiprocess = NULL;
  char *CallProcessName = PsGetProcessImageFileName(PsGetCurrentProcess());
  if (strstr(CallProcessName, "System") != NULL ||
      PsGetCurrentProcessId() == 4) {
...
__int64 __fastcall proxyObpCallPreOperationCallbacks(__int64 a1, __int64 a2,
                                                     __int64 a3) {
  if (VtMode) {
  ...
  if (Padd == NULL) {
    ...
    return ((pfObpCallPreOperationCallbacks)ori_pslp9)(
        a1, a2, a3); //不是我们的进程操作进程对象就调用原来函数
  ...
  return ((pfObpCallPreOperationCallbacks)ori_pslp9)(
      a1, a2, a3); //不是我们的进程操作进程对象就调用原来函数return 0;//放行
}
VOID proxyListFunc(_In_ HANDLE ParentId, _In_ HANDLE ProcessId,
                   _In_ BOOLEAN Create) {
...
NTSTATUS myNtSetInformationThread(HANDLE threadHandle, THREADINFOCLASS
threadInformationClass, PVOID threadInformation, ULONG threadInformationLength){
    ...
    if (Padd == NULL)
    {
        status = NtSetInformationThread(threadHandle, threadInformationClass,
...
threadInformationLength == 0x2CC && threadInformation != NULL && threadHandle !=
NULL)//ThreadWow64Context
        {
            Sstatus = ObReferenceObjectByHandle(threadHandle,
                THREAD_ALL_ACCESS,
                *PsThreadType,
            ...
            if ((pframe != NULL) && MmIsAddressValid(&pframe->Dr0) &&
MmIsAddressValid(&pframe->Dr1) && MmIsAddressValid(&pframe->Dr2) &&
MmIsAddressValid(&pframe->Dr3) && MmIsAddressValid(&pframe->Dr6) &&
MmIsAddressValid(&pframe->Dr7))
            {
                if (contex->Dr7 != NULL)
                {
...
threadInformationClass, threadInformation, threadInformationLength);
            ...
            DbgPrint("SET thread: %p dr0: %d dr1 :%d dr2 :%d dr3 :%d dr6:%d
dr7:%d", Thread, contex->Dr0, contex->Dr1, contex->Dr2, contex->Dr3,
contex->Dr6, contex->Dr7);
    ...
    if (threadInformationClass == ThreadWow64Context && threadInformationLength
== 0x2CC && threadInformation != NULL && threadHandle !=
NULL)//ThreadWow64Context
    {
        Sstatus = ObReferenceObjectByHandle(threadHandle,
            THREAD_ALL_ACCESS,
...
NTSTATUS __fastcall myNtQueryInformationThread(IN   HANDLE   ThreadHandle,
    ...
    if (Padd == NULL)
    {
        status = orgNtQueryInformationThread(ThreadHandle,
ThreadInformationClass, ThreadInformation, ThreadInformationLength,
ReturnLength);
            ...
            if (Process != NULL){
                ...
                if (PlIST != NULL)
                {
                    ...
                    if (TList != NULL)
                    {
                        ...
                        DbgPrint(" QUERY thread: %p dr0: %d dr1 :%d dr2 :%d dr3
:%d dr6:%d dr7:%d", Thread, contex->Dr0, contex->Dr1, contex->Dr2, contex->Dr3,
contex->Dr6, contex->Dr7);
...
NTSTATUS __fastcall myNtQueryInformationThread(
    IN HANDLE ThreadHandle, IN THREADINFOCLASS ThreadInformationClass,
    OUT PVOID ThreadInformation, IN ULONG ThreadInformationLength,
    OUT PULONG ReturnLength OPTIONAL) {
  ...
  Padd = querylist(PmainList, PsGetCurrentProcessId(), PsGetCurrentProcess());
  status = orgNtQueryInformationThread(ThreadHandle, ThreadInformationClass,
                                       ThreadInformation,
  ...
  if (ThreadInformationClass == ThreadWow64Context &&
      ThreadInformationLength == 0x2CC && ThreadInformation != NULL &&
      ThreadHandle != NULL) // ThreadWow64Context
  {
    Sstatus = ObReferenceObjectByHandle(ThreadHandle, THREAD_ALL_ACCESS,
...
void *__fastcall myRtlpCopyLegacyContextX86(BOOLEAN islegacy,
                                            PWOW64_CONTEXT destcontex,
                                            ULONG nouse_falg,
                                            PWOW64_CONTEXT srccontext) {
  ...
  if (islegacy) {
    ...
    result = memmove(&destcontex->ExtendedRegisters,
                     &srccontext->ExtendedRegisters, 0x200ui64);
...
VOID myKeContextFromKframes(PKTRAP_FRAME TrapFrame,
                            IN PKEXCEPTION_FRAME ExceptionFrame,
                            PCONTEXT ContextFrame) {}
VOID __fastcall myKeContextToKframes(PKTRAP_FRAME TrapFrame,
                                     IN PKEXCEPTION_FRAME ExceptionFrame,
                                     PCONTEXT ContextFrame, ULONG CONTEXTFALG,
                                     KPROCESSOR_MODE rMODE) {}
///////////
...
BOOLEAN IsProtectedProcess(PEPROCESS eprocess) {
  ...
  /*if (strstr(processName, "BlackCipher") != NULL && strstr(processName2,
  "cstrike-on") != NULL)///NGS Cant Read and Write Process MEmory!
  {
...
preCall(PVOID RegistrationContext,
...
#define PROCESS_TERMINATE 0x1
  ...
  if (IsProtectedProcess((PEPROCESS)pOperationInformation->Object)) {
    ...
    if (pOperationInformation->Operation == OB_OPERATION_HANDLE_DUPLICATE) {
      ...
      if ((pOperationInformation->Parameters->DuplicateHandleInformation
               .OriginalDesiredAccess &
           PROCESS_TERMINATE) == PROCESS_TERMINATE) {
...
OB_PREOP_CALLBACK_STATUS
preCall2(PVOID RegistrationContext,
         POB_PRE_OPERATION_INFORMATION pOperationInformation) {
  ...
  if (IsProtectedProcess(ep)) {
    if (pOperationInformation->Operation == OB_OPERATION_HANDLE_CREATE) {
      ...
      if ((pOperationInformation->Parameters->CreateHandleInformation
        ...
        pOperationInformation->Parameters->CreateHandleInformation
            .DesiredAccess &= ~THREAD_TERMINATE2;
        pOperationInformation->Parameters->CreateHandleInformation
...
VOID RwWorkItem(_In_ PDEVICE_OBJECT DeviceObject,
  ...
  if (Data->IsReadOrWrite)
  {
      ...
      //读
      Data->Status = PreadMem(Data->ProcessHandle, Data->BaseAddress,
  Data->Buffer, Data->BufferLength, Data->ReturnLength);
  }
  else
  {
      ...
      Data->Status = PwriteMem(Data->ProcessHandle, Data->BaseAddress,
  Data->Buffer, Data->BufferLength, Data->ReturnLength);
  }
...
VOID ReadOrWriteR3Mem(PReadWriteR3MemData Data) {
  ...
  /*
      ...
      if (pIoWorkItem)
      {
          IoQueueWorkItem(pIoWorkItem, (PIO_WORKITEM_ROUTINE)RwWorkItem,
  ...
  if (NT_SUCCESS(status)) {
    ...
    KeWaitForSingleObject(&KwaitForReadOrWrite, Executive, KernelMode, FALSE,
                          NULL);
  }
...
NTSTATUS __fastcall MyNtReadVirtualMemory(IN HANDLE ProcessHandle,
                                          IN PVOID BaseAddress,
                                          OUT PVOID Buffer,
                                          IN ULONG BufferLength,
                                          OUT PULONG ReturnLength OPTIONAL) {
  ...
  return ori_pslp27(ProcessHandle, BaseAddress, Buffer, BufferLength,
                    ReturnLength);
  ...
  if (Pdata==NULL)
  {
      return  PreadMem(ProcessHandle, BaseAddress, Buffer, BufferLength,
  ReturnLength);
  ...
  return NtReadVirtualMemory(ProcessHandle, BaseAddress, Buffer, BufferLength,
  ReturnLength);
  //yes
  }
  else{
...
NTSTATUS __fastcall MyNtWriteVirtualMemory(IN HANDLE ProcessHandle,
                                           IN PVOID BaseAddress,
                                           IN PVOID Buffer,
                                           IN ULONG BufferLength,
                                           OUT PULONG ReturnLength OPTIONAL) {
  ...
  return ori_pslp28(ProcessHandle, BaseAddress, Buffer, BufferLength,
                    ReturnLength);
  /*
      PReadWriteR3MemData Pdata = NULL;
      p_save_handlentry Padd = NULL;
      ...
      if (Pdata == NULL)
      {
          return  PwriteMem(ProcessHandle, BaseAddress, Buffer, BufferLength,
     ReturnLength);
      }
      else{
...
BOOLEAN IsSystemProcessCall() {
  ...
  if (Process != NULL) {
    ...
    if (MmIsAddressValid(processName) == TRUE) {
      if (strstr(processName, "csrss") != NULL ||
          strstr(processName, "explorer") != NULL ||
          strstr(processName, "svchost") != NULL) {
...
ULONG __fastcall MyNtOpenProcess(__out PHANDLE ProcessHandle,
                                 __in ACCESS_MASK AccessMask,
                                 __in PVOID ObjectAttributes,
                                 __in PCLIENT_ID ClientId) {
  p_save_handlentry Padd = NULL;
  ...
  if (IsSystemProcessCall()) {
    return XNtOpenProcess(ProcessHandle, AccessMask, ObjectAttributes,
                          ClientId); //调用原函数
  }
...
NTSTATUS __fastcall proxyPsSetContextThread(__in PETHREAD Thread,
...
NTSTATUS __fastcall proxyPspGetContextThreadInternal(
    __in PETHREAD Ethread, __in PCONTEXT ThreadContext,
    __in KPROCESSOR_MODE PreviousMode, __in BOOLEAN isSystemThread,
    __in BOOLEAN dwOne) {}
...
NTSTATUS __fastcall proxyPsLookupThreadByThreadId(_In_ HANDLE ThreadId,
                                                  _Outptr_ PETHREAD *Thread) {
...
NTSTATUS __fastcall proxyPsLookupProcessByProcessId(_In_ HANDLE ProcessId,
                                                    _Out_ PEPROCESS *Process) {
  if (VtMode) {
    PPAGE_HOOK_ENTRY pEntry =
        PHGetHookEntry(fc_DbgkGetAdrress(L"PsLookupProcessByProcessId"));
...
ULONG64 __fastcall proxyExGetCallBackBlockRoutine(ULONG64 a1) {
  KIRQL oldirql = 0;
  KeAcquireSpinLock(&local_lock,
                    &oldirql); //加锁 不加会有些无良驱动来回注册 会蓝的很难看
...
ULONG64 __fastcall proxyExCompareExchangeCallBack(ULONG64 a1, ULONG64 a2,
                                                  ULONG64 a3) {
  p_save_handlentry Padd;
  Padd = querylist(PmainList, PsGetCurrentProcessId(), PsGetCurrentProcess());
  if (Padd != NULL) {
...
VOID T_KiSaveDebugRegisterState() { return 0; }
...
VOID initANti() {
  ...
  pslp_head_n_byte9 = HookKernelApi(ObpCallPreOperationCallbacks,
                                    ...
                                    &ori_pslp9, &pslp_patch_size9);

  pslp_head_n_byte25 =
      HookKernelApi(RtlpCopyLegacyContextX86, (PVOID)myRtlpCopyLegacyContextX86,
                    &ori_pslp25, &pslp_patch_size25);

  pslp_head_n_byte27 =
      HookKernelApi(NtReadVirtualMemory, (PVOID)MyNtReadVirtualMemory,
                    &ori_pslp27, &pslp_patch_size27);
  pslp_head_n_byte28 =
      HookKernelApi(NtWriteVirtualMemory, (PVOID)MyNtWriteVirtualMemory,
                    &ori_pslp28, &pslp_patch_size28);
  ...
  */
  ...
  /*
      pslp_head_n_byte21 =
     HookKernelApi(fc_DbgkGetAdrress(L"PsLookupThreadByThreadId") ,
          (PVOID)proxyPsLookupThreadByThreadId,
          &ori_pslp21,
          &pslp_patch_size21);
          */
  pslp_head_n_byte22 = HookKernelApi(
      fc_DbgkGetAdrress(L"PsLookupProcessByProcessId"),
      (PVOID)proxyPsLookupProcessByProcessId, &ori_pslp22, &pslp_patch_size22);
  pslp_head_n_byte19 =
      HookKernelApi(NtQueryInformationThread, (PVOID)myNtQueryInformationThread,
                    &ori_pslp19, &pslp_patch_size19);
...
VOID unload() {
  ...
  UnhookKernelApi(ObpCallPreOperationCallbacks, pslp_head_n_byte9,
                  pslp_patch_size9);
  UnhookKernelApi(NtQueryInformationThread, pslp_head_n_byte19,
                  pslp_patch_size19);
  UnhookKernelApi(RtlpCopyLegacyContextX86, pslp_head_n_byte25,
                  pslp_patch_size25);
  //  UnhookKernelApi(KiAttachProcess, pslp_head_n_byte26, pslp_patch_size26);
  UnhookKernelApi(fc_DbgkGetAdrress(L"PsLookupProcessByProcessId"),
                  pslp_head_n_byte22, pslp_patch_size22);
...
/*
  UnhookKernelApi(KiSaveDebugRegisterState, pslp_head_n_byte24,
pslp_patch_size24); UnhookKernelApi(KiRestoreDebugRegisterState,
pslp_head_n_byte23, pslp_patch_size23);*/
  /*
      UnhookKernelApi(ExCompareExchangeCallBack, pslp_head_n_byte20,
     pslp_patch_size20);
  */

  /*
      UnhookKernelApi(fc_DbgkGetAdrress(L"PsLookupThreadByThreadId"),
     pslp_head_n_byte21, pslp_patch_size21);
      */

  //  UnhookKernelApi(ExGetCallBackBlockRoutine, pslp_head_n_byte10,
  //  pslp_patch_size10);
...
VOID EPT_InitAnti() {
  ...
  PHHook(fc_DbgkGetAdrress(L"PsLookupProcessByProcessId"),
         proxyPsLookupProcessByProcessId);
...
BOOLEAN VxkCopyMemory(PVOID pDestination, PVOID pSourceAddress,
                      SIZE_T SizeOfCopy) {
  PMDL pMdl = NULL;
...
VOID LoadImageNotifyRoutine(__in_opt PUNICODE_STRING FullImageName,
                            __in HANDLE ProcessId, __in PIMAGE_INFO ImageInfo) {
  PVOID pDrvEntry;
