...
VOID PHpHookCallbackDPC(IN PRKDPC Dpc, IN PVOID Context,
                        IN PVOID SystemArgument1, IN PVOID SystemArgument2) {
  UNREFERENCED_PARAMETER(Dpc);
  PHOOK_CONTEXT pCTX = (PHOOK_CONTEXT)Context;

  if (pCTX != NULL)
    __vmx_vmcall(pCTX->Hook ? HYPERCALL_HOOK_PAGE : HYPERCALL_UNHOOK_PAGE,
                 pCTX->DataPagePFN, pCTX->CodePagePFN, 0);
...
NTSTATUS PHpCopyCode(IN PVOID pFunc, OUT PUCHAR OriginalStore,
                     OUT PULONG pSize) {
  ...
  do {
    ...
    if (ld.flags & F_INVALID ||
        (len == 1 &&
         (src[ld.opcd_offset] == 0xCC || src[ld.opcd_offset] == 0xC3)) ||
        (len == 3 && src[ld.opcd_offset] == 0xC2) || len + all_len > 128) {
      break;
    }
    ...
    if (ld.flags & F_RELATIVE) {
      LONG diff = 0;
      const uintptr_t ofst =
          (ld.disp_offset != 0 ? ld.disp_offset : ld.imm_offset);
...
NTSTATUS PHHook(IN PVOID pFunc, IN PVOID pHook) {
  ...
  PPAGE_HOOK_ENTRY pHookEntry =
      ExAllocatePoolWithTag(NonPagedPool, sizeof(PAGE_HOOK_ENTRY), HB_POOL_TAG);
  if (pHookEntry == NULL)
    return STATUS_INSUFFICIENT_RESOURCES;
  ...
  // Copy original code
  NTSTATUS status =
      PHpCopyCode(pFunc, pHookEntry->OriginalData, &pHookEntry->OriginalSize);
...
NTSTATUS PHRestore(IN PVOID pFunc) {
  ...
  if (PHPageHookCount(pFunc, DATA_PAGE) > 1) {
    // TODO: atomic memory patching, i.e. with other CPUs spinlocked
    ULONG_PTR page_offset = (ULONG_PTR)pFunc - (ULONG_PTR)PAGE_ALIGN(pFunc);
    memcpy((PUCHAR)pHookEntry->CodePageVA + page_offset,
           pHookEntry->OriginalData, pHookEntry->OriginalSize);
...
PPAGE_HOOK_ENTRY PHGetHookEntry(IN PVOID ptr) {
  ...
  for (PLIST_ENTRY pListEntry = g_PageList.Flink; pListEntry != &g_PageList;
       pListEntry = pListEntry->Flink) {
    PPAGE_HOOK_ENTRY pEntry =
        CONTAINING_RECORD(pListEntry, PAGE_HOOK_ENTRY, Link);
...
ULONG PHPageHookCount(IN PVOID ptr, IN PAGE_TYPE Type) {
  ...
  for (PLIST_ENTRY pListEntry = g_PageList.Flink; pListEntry != &g_PageList;
    ...
    PPAGE_HOOK_ENTRY pEntry =
        CONTAINING_RECORD(pListEntry, PAGE_HOOK_ENTRY, Link);
    if ((Type == DATA_PAGE && pEntry->DataPageVA == page) ||
        (Type == CODE_PAGE && pEntry->CodePageVA == page))
      count++;
  }
...
PPAGE_HOOK_ENTRY PHGetHookEntryByPFN(IN ULONG64 pfn, IN PAGE_TYPE Type) {
  ...
  for (PLIST_ENTRY pListEntry = g_PageList.Flink; pListEntry != &g_PageList;
    ...
    PPAGE_HOOK_ENTRY pEntry =
        CONTAINING_RECORD(pListEntry, PAGE_HOOK_ENTRY, Link);
    if ((Type == DATA_PAGE && pEntry->DataPagePFN == pfn) ||
        (Type == CODE_PAGE && pEntry->CodePagePFN == pfn))
      return pEntry;
  }
