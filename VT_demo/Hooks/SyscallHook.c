...
VOID SHpHookCallbackDPC(PRKDPC Dpc, PVOID Context, PVOID SystemArgument1,
                        PVOID SystemArgument2) {
  UNREFERENCED_PARAMETER(Dpc);

  __vmx_vmcall(Context != NULL ? HYPERCALL_HOOK_LSTAR : HYPERCALL_UNHOOK_LSTAR,
               (ULONG64)Context, 0, 0);
...
NTSTATUS SHInitHook() {
  ...
  if (KiServiceCopyEndPtr == 0) {
    CHAR pattern[] = "\xF7\x05\xcc\xcc\xcc\xcc\xcc\xcc\xcc\xcc\x0F\x85\xcc\xcc"
                     "\xcc\xcc\x41\xFF\xD2";
    status = UtilScanSection(".text", (PCUCHAR)pattern, 0xCC,
